...
import type {
  ...
  WeekSixPromptWorkV1,
  WeekSixFactCheckCompletionEvidence,
  WeekSixFactCheckMissionSession,
  WeekSixFactCheckWorkV1,
  WeekSixArchiveCompletionEvidence,
  WeekSixArchiveMissionSession,
  WeekSixArchiveWorkV1,
  ProgressV3,
} from './types';
...
import { MISSION_IDS,MISSION_ORDER } from './missionCatalog';
...
function formalWeekSixClassificationCompletionEvidence(session:WeekSixClassificationMissionSession|undefined,source:WeekSixRecordsWorkV1|undefined,completedAt:string,createdAt:string,verifiedAt:string):{evidence:Extract<WeekSixClassificationCompletionEvidence,{kind:'formal-v3'}>;work:WeekSixClassificationWorkV1}|null{if(!session||!source||!session.lastRun||!session.lastCheckedAt)return null;const run=runWeekSixClassification(session.input,session.sourceRows);if(!run.completed||run.state!=='classification-proven'||session.failureSnapshot!==null||!deeplyEqual(run,session.lastRun)||session.sourceWorkId!==source.workId||session.sourceVerifiedAt!==source.verifiedAt||!deeplyEqual(session.sourceRows,source.run.rows))return null;const work:WeekSixClassificationWorkV1={kind:'ai-evidence-classification-v1',workId:'w6-m2-fan-evidence-classification',missionId:'w6-m2',title:'真假扇证据分类记录',sourceWorkId:source.workId,sourceVerifiedAt:source.verifiedAt,sourceRows:structuredClone(session.sourceRows),input:structuredClone(session.input),run:structuredClone(run),createdAt,verifiedAt};return{work,evidence:{kind:'formal-v3',completedAt,verifiedAt,sourceWorkId:source.workId,sourceVerifiedAt:source.verifiedAt,sourceRows:structuredClone(session.sourceRows),input:structuredClone(session.input),run:structuredClone(run),workId:work.workId}};}

function hasValidFormalWeekSixClassificationCompletion(progress:ProgressV3):boolean{const mission=progress.missions['w6-m2'],rawEvidence=progress.missionCompletionEvidence['w6-m2'],rawSession=progress.sessions['w6-m2'],rawWork=progress.works['w6-m2-fan-evidence-classification'],source=progress.works['w6-m1-structured-records-table'];if(!hasValidFormalWeekSixRecordsCompletion(progress)||!hasValidCompletedMission(mission)||rawEvidence?.kind!=='formal-v3'||!rawSession||!rawWork||!source)return false;try{const session=parseWeekSixClassificationSession(rawSession),work=parseWeekSixClassificationWork(rawWork);return parseWeekSixClassificationEvidence(rawEvidence,{mission,formalWeekSixRecords:true,session,work,sourceWork:source}).kind==='formal-v3';}catch{return false;}}
...
function formalWeekSixFactCheckCompletionEvidence(session:WeekSixFactCheckMissionSession|undefined,sourceWork:WeekSixPromptWorkV1|undefined,completedAt:string,createdAt:string,verifiedAt:string):{evidence:Extract<WeekSixFactCheckCompletionEvidence,{kind:'formal-v3'}>;work:WeekSixFactCheckWorkV1}|null{if(!session||!sourceWork||!session.lastRun||!session.lastRunAt)return null;const source=deriveWeekSixFactCheckSource(sourceWork),run=runWeekSixFactCheck(session.input,source);if(!run.completed||run.state!=='review-proven'||session.failureSnapshot!==null||!deeplyEqual(run,session.lastRun)||!deeplyEqual(session.source,source))return null;const work:WeekSixFactCheckWorkV1={kind:'ai-fact-check-report-v1',workId:'w6-m4-second-attempt-review',missionId:'w6-m4',title:'二调芭蕉扇回答审校报告',sourceWorkId:sourceWork.workId,sourceVerifiedAt:sourceWork.verifiedAt,source:structuredClone(source),input:structuredClone(session.input),run:structuredClone(run),createdAt,verifiedAt};return{work,evidence:{kind:'formal-v3',completedAt,verifiedAt,sourceWorkId:sourceWork.workId,sourceVerifiedAt:sourceWork.verifiedAt,source:structuredClone(source),input:structuredClone(session.input),run:structuredClone(run),workId:work.workId}}}
function hasValidFormalWeekSixFactCheckCompletion(progress:ProgressV3):boolean{const mission=progress.missions['w6-m4'],rawEvidence=progress.missionCompletionEvidence['w6-m4'],rawSession=progress.sessions['w6-m4'],rawWork=progress.works['w6-m4-second-attempt-review'],source=progress.works['w6-m3-second-attempt-brief'];if(!hasValidFormalWeekSixPromptCompletion(progress)||!hasValidCompletedMission(mission)||rawEvidence?.kind!=='formal-v3'||!rawSession||!rawWork||!source)return false;try{const session=parseWeekSixFactCheckSession(rawSession),work=parseWeekSixFactCheckWork(rawWork);return parseWeekSixFactCheckEvidence(rawEvidence,{mission,formalWeekSixPrompt:true,session,work,sourceWork:source}).kind==='formal-v3'}catch{return false}}
export function completeWeekSixFactCheckProgress(progress:ProgressV3,input:CompletionInput):ProgressV3{const previous=progress.missions['w6-m4'],existing=progress.missionCompletionEvidence['w6-m4'];if(existing?.kind==='formal-v3'){if(hasValidFormalWeekSixFactCheckCompletion(progress))return progress;throw Error('W6-M4完成无效')}if(!hasValidFormalWeekSixPromptCompletion(progress))throw Error('W6-M4完成无效');if(previous&&existing?.kind!=='legacy-replay-only')throw Error('W6-M4完成无效');const now=new Date().toISOString(),completion=formalWeekSixFactCheckCompletionEvidence(progress.sessions['w6-m4'],progress.works['w6-m3-second-attempt-brief'],previous?.completedAt??now,now,now);if(!completion)throw Error('W6-M4完成无效');const mission=previous??{status:'completed' as const,stars:normalizeStars(input.stars),attempts:safeCount(0,1),hintsUsed:safeCount(0,normalizeHints(input.hintsUsed)),completedAt:now};return{...progress,missions:{...progress.missions,'w6-m4':mission},missionCompletionEvidence:{...progress.missionCompletionEvidence,'w6-m4':completion.evidence},works:{...progress.works,[completion.work.workId]:completion.work},savedAt:now}}
function formalWeekSixArchiveCompletionEvidence(session:WeekSixArchiveMissionSession|undefined,sourceWork:WeekSixFactCheckWorkV1|undefined,completedAt:string,createdAt:string,verifiedAt:string):{evidence:Extract<WeekSixArchiveCompletionEvidence,{kind:'formal-v3'}>;work:WeekSixArchiveWorkV1}|null{if(!session||!sourceWork||!session.lastRun||!session.lastRunAt||!session.workerTrace||!session.workerRun)return null;const source=deriveWeekSixArchiveSource(sourceWork),run=runWeekSixArchiveCore(session.pythonCode,{trace:session.workerTrace,run:session.workerRun},session.input,source);if(!run.completed||run.state!=='archive-proven'||session.failureSnapshot!==null||!deeplyEqual(run,session.lastRun)||!deeplyEqual(session.source,source))return null;const shared={source:structuredClone(source),pythonCode:session.pythonCode,workerTrace:structuredClone(session.workerTrace),workerRun:structuredClone(session.workerRun),input:structuredClone(session.input),run:structuredClone(run)};const work:WeekSixArchiveWorkV1={kind:'python-ai-archive-v1',workId:'w6-m5-journey-archive',missionId:'w6-m5',title:'三调芭蕉扇取经档案',...shared,createdAt,verifiedAt};return{work,evidence:{kind:'formal-v3',completedAt,verifiedAt,workId:work.workId}}}
export function hasValidFormalWeekSixArchiveCompletion(progress:ProgressV3):boolean{const mission=progress.missions['w6-m5'],rawEvidence=progress.missionCompletionEvidence['w6-m5'],rawSession=progress.sessions['w6-m5'],rawWork=progress.works['w6-m5-journey-archive'],source=progress.works['w6-m4-second-attempt-review'];if(!hasValidFormalWeekSixFactCheckCompletion(progress)||!hasValidCompletedMission(mission)||rawEvidence?.kind!=='formal-v3'||!rawSession||!rawWork||!source)return false;try{const session=parseWeekSixArchiveSession(rawSession),work=parseWeekSixArchiveWork(rawWork);return parseWeekSixArchiveEvidence(rawEvidence,{mission,formalWeekSixFactCheck:true,session,work,sourceWork:source}).kind==='formal-v3'}catch{return false}}
export function completeWeekSixArchiveProgress(progress:ProgressV3,input:CompletionInput):ProgressV3{const previous=progress.missions['w6-m5'],existing=progress.missionCompletionEvidence['w6-m5'];if(existing?.kind==='formal-v3'){if(hasValidFormalWeekSixArchiveCompletion(progress))return progress;throw Error('W6-M5完成无效')}if(!hasValidFormalWeekSixFactCheckCompletion(progress))throw Error('W6-M5完成无效');if(previous&&existing?.kind!=='legacy-replay-only')throw Error('W6-M5完成无效');const now=new Date().toISOString(),completion=formalWeekSixArchiveCompletionEvidence(progress.sessions['w6-m5'],progress.works['w6-m4-second-attempt-review'],previous?.completedAt??now,now,now);if(!completion)throw Error('W6-M5完成无效');const mission=previous??{status:'completed' as const,stars:normalizeStars(input.stars),attempts:safeCount(0,1),hintsUsed:safeCount(0,normalizeHints(input.hintsUsed)),completedAt:now};return{...progress,missions:{...progress.missions,'w6-m5':mission},missionCompletionEvidence:{...progress.missionCompletionEvidence,'w6-m5':completion.evidence},works:{...progress.works,[completion.work.workId]:completion.work},savedAt:now}}

export function completeMission(progress: ProgressV3, missionId: string, input: CompletionInput): ProgressV3 {
  if (!MISSION_IDS.has(missionId)) throw new Error('任务编号无效');
  ...
  if (missionId === 'w6-m4') return completeWeekSixFactCheckProgress(progress, input);
  if (missionId === 'w6-m5') return completeWeekSixArchiveProgress(progress, input);
...
export function isMissionUnlocked(progress: ProgressV3, missionId: string): boolean {
  ...
  if (missionId === 'w6-m4') { const access=getWeekSixFactCheckAccess(progress);return access.kind==='formal'||(access.kind==='historical-read-only'&&access.completed); }
  if (missionId === 'w6-m5') return getWeekSixArchiveAccess(progress).kind!=='locked';
  return progress.missions[MISSION_ORDER[index - 1]!] ?.status === 'completed';
}
...
export type WeekSixFactCheckAccess={kind:'locked'}|{kind:'historical-read-only';completed:boolean}|{kind:'formal';upgradingLegacy:boolean};
export function getWeekSixFactCheckAccess(progress:ProgressV3):WeekSixFactCheckAccess{const evidence=progress.missionCompletionEvidence['w6-m4'];if(hasValidFormalWeekSixPromptCompletion(progress))return{kind:'formal',upgradingLegacy:evidence?.kind==='legacy-replay-only'};const prerequisite=progress.missionCompletionEvidence['w6-m3'];if((progress.missions['w6-m3']?.status==='completed'&&prerequisite?.kind==='legacy-replay-only')||evidence?.kind==='legacy-replay-only')return{kind:'historical-read-only',completed:progress.missions['w6-m4']?.status==='completed'&&evidence?.kind==='legacy-replay-only'};return{kind:'locked'}}
export type WeekSixArchiveAccess={kind:'locked'}|{kind:'historical-read-only';completed:boolean}|{kind:'formal';upgradingLegacy:boolean};
export function getWeekSixArchiveAccess(progress:ProgressV3):WeekSixArchiveAccess{const evidence=progress.missionCompletionEvidence['w6-m5'];if(hasValidFormalWeekSixFactCheckCompletion(progress))return{kind:'formal',upgradingLegacy:evidence?.kind==='legacy-replay-only'};return evidence?.kind==='legacy-replay-only'?{kind:'historical-read-only',completed:!!progress.missions['w6-m5']}:{kind:'locked'}}
