...
describe('W5-M2 durable function chain', () => {
  ...
  it('requires formal W5-M1, atomically seals work/proof, survives round-trip and unlocks W5-M3', () => {
    ...
    expect(completed.schemaRevision).toBe(14); expect(completed.missionCompletionEvidence['w5-m2']?.kind).toBe('formal-v3'); expect(completed.works['w5-m2-sanqing-function-record']?.run.completed).toBe(true);
  ...
  it('migrates revision 13 without damaging formal W5-M1 or fabricating W5-M2 evidence', () => {
    ...
    const migrated = migrateProgress(old); expect(migrated.schemaRevision).toBe(14); expect(migrated.missionCompletionEvidence['w5-m1']).toEqual(current.missionCompletionEvidence['w5-m1']); expect(migrated.sessions['w5-m2']).toBeUndefined();
