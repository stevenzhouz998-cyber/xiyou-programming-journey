...
import { WEEK_FOUR_BOSS_CARDS, type WeekFourBossRunResult, type WeekFourBossTraceItem } from '../engine/weekFourBossContract';
import { parseWeekFourBossPython } from '../engine/weekFourBossPythonGrammar';
import { createWeekFourBossPythonRuntime, WeekFourBossRuntimeError, type WeekFourBossPythonRuntime } from '../engine/weekFourBossPythonRunner';
import { createMissionSession } from '../progress/session';
import type { CoordinatedSaveResult } from '../progress/storageCoordinator';
import type { WeekFourBossMissionSession } from '../progress/weekFourBossSession';
import { downloadTextFile } from '../utils/download';
import { LazySectionBoundary } from './LazySectionBoundary';
import type { WeekFourBossPythonEditorHandle, WeekFourBossPythonEditorProps } from './WeekFourBossPythonEditor';
import type { WeekFourBossSceneProps } from './WeekFourBossScene';

import './WeekFourBossExperience.css';

const loadWeekFourBossPythonEditor = () => import('./WeekFourBossPythonEditor').then((module) => ({ default: module.WeekFourBossPythonEditor }));
const loadWeekFourBossScene = () => import('./WeekFourBossScene').then((module) => ({ default: module.WeekFourBossScene }));

export interface WeekFourBossExperienceProps {
  reducedMotion: boolean;
  muted: boolean;
  locked?: boolean;
  onComplete(evidence: { stars: 1 | 2 | 3; hintsUsed: number }): void | boolean | Promise<boolean>;
  runtimeFactory?: () => WeekFourBossPythonRuntime;
...
type BranchRunInput = { canonicalTrace: WeekFourBossTraceItem[]; workerTrace: WeekFourBossTraceItem[]; run: WeekFourBossRunResult };
type RejectedWrite =
...
function feedbackFor(run: WeekFourBossRunResult): string {
  if (run.state === 'identity-conflict') return '读入变量的内容与公开身份不符；首次偏离的卡片事实已保存。';
  if (run.state === 'branch-conflict') return '有的卡片走了不合适的分支；实际条件与动作已保存。';
  return '两轮公开卡片均已正确核验。';
}

export function WeekFourBossExperience({
  reducedMotion,
  muted,
  locked = false,
  onComplete,
  runtimeFactory = createWeekFourBossPythonRuntime,
  onSessionPersistenceActiveChange,
  onInteractionLockChange,
}: WeekFourBossExperienceProps) {
  const context = useProgress();
  const persistedInitial = context.progress.sessions['w4-m5'];
  const initial = useMemo<WeekFourBossMissionSession>(() => persistedInitial ?? createMissionSession('w4-m5', new Date().toISOString()), []);
  ...
  const [message, setMessage] = useState(persistedInitial ? (initial.lastRun?.completed ? '核验报告和正式证明已保存；可以重新运行核验。' : initial.lastRun ? feedbackFor(initial.lastRun) : '先运行核验程序，对照两轮卡片读入的身份和实际动作。') : '正在保存默认 Python 草稿…');
  ...
  const [formalCompleted, setFormalCompleted] = useState(context.progress.missionCompletionEvidence['w4-m5']?.kind === 'formal-v3');
  ...
  const runtimeRef = useRef<WeekFourBossPythonRuntime | null>(null);
  const editorRef = useRef<WeekFourBossPythonEditorHandle | null>(null);
  ...
  const pendingRunRef = useRef<{ canonicalTrace: WeekFourBossTraceItem[]; workerTrace: WeekFourBossTraceItem[]; run: WeekFourBossRunResult } | null>(null);
  ...
  const Editor = useMemo(() => lazy(loadWeekFourBossPythonEditor) as React.ComponentType<WeekFourBossPythonEditorProps & React.RefAttributes<WeekFourBossPythonEditorHandle>>, [editorGeneration]);
  const Scene = useMemo(() => lazy(loadWeekFourBossScene) as React.ComponentType<WeekFourBossSceneProps>, []);
  ...
  const accept = (result: CoordinatedSaveResult, expectedGeneration: number): WeekFourBossMissionSession | null => {
    if (!mountedRef.current || expectedGeneration !== revisionRef.current) return null;
    if (result.status !== 'saved') return null;
    const next = result.progress.sessions['w4-m5'];
    if (!next) return null;
  ...
  const finishFormalCompletion = async (result: CoordinatedSaveResult, generation: number) => {
    ...
    const savedMission = result.progress.missions?.['w4-m5'];
    const proof = result.progress.missionCompletionEvidence?.['w4-m5'];
    const savedWork = result.progress.works?.['w4-m5-verification-report'];
    ...
    if (result.status !== 'saved' || proof?.kind !== 'formal-v3' || savedWork?.kind !== 'python-verification-station-v1') {
      completionActiveRef.current = false;
  ...
  const completeSolved = async () => {
    ...
    const attempt = await attemptContextWrite(() => context.completeWeekFourBoss(input));
    if (!mountedRef.current || generation !== revisionRef.current) return;
    if (!attempt.ok) {
  ...
  useEffect(() => {
    ...
    setLock(true, 'session-pending');
    void (async () => {
      const attempt = await attemptContextWrite(() => context.saveWeekFourBossDraft(initial.pythonCode));
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (!attempt.ok) {
      ...
      setMessage('默认 Python 草稿已安全保存，可以运行两轮核验。');
      finish();
    })();
  }, []);
  ...
  const edit = async (code: string) => {
    ...
    setLock(true, 'session-pending');
    const attempt = await attemptContextWrite(() => context.saveWeekFourBossDraft(code));
    if (!mountedRef.current || generation !== revisionRef.current) return;
    if (!attempt.ok) {
  ...
  const replay = async () => {
    ...
    try {
      ...
      const mission = context.progress.missions['w4-m5'];
      const prerequisite = context.progress.missionCompletionEvidence['w4-m4'];
      const proof = context.progress.missionCompletionEvidence['w4-m5'];
      const savedWork = context.progress.works['w4-m5-verification-report'];
      ...
      if (!saved.lastRun?.completed
        ...
        || proof.workId !== 'w4-m5-verification-report'
        || !savedWork
        || savedWork.kind !== 'python-verification-station-v1'
        || savedWork.workId !== 'w4-m5-verification-report'
        || savedWork.missionId !== 'w4-m5'
        || saved.kind !== 'python-verification-station-v1'
      ...
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (error instanceof WeekFourBossRuntimeError && !['validation', 'busy'].includes(error.code)) setRuntimeReady(false);
  ...
  const run = async () => {
    ...
    const draftAttempt = await attemptContextWrite(() => context.saveWeekFourBossDraft(candidate));
    if (!mountedRef.current || generation !== revisionRef.current) return;
    if (!draftAttempt.ok) {
    ...
    let parsed;
    try { parsed = parseWeekFourBossPython(saved.pythonCode); }
    catch {
    ...
    if ('state' in parsed) {
      const validationAttempt = await attemptContextWrite(() => context.saveWeekFourBossValidationFailure());
      if (!mountedRef.current || generation !== revisionRef.current) return;
    ...
    try {
      ...
      const runAttempt = await attemptContextWrite(() => context.saveWeekFourBossRun(input));
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (!runAttempt.ok) {
        pendingRunRef.current = input;
        recoverRejected({ kind: 'run', stage: 'run', input, message: '核验运行结果保存异常，场景没有发布这次结果。' });
        return;
      }
      ...
      if (!savedRun) {
        pendingRunRef.current = input;
        recover('run', runResult.status === 'conflict' ? 'conflict' : 'unsaved', '核验运行结果尚未保存，当前场景不会把它当作事实。');
        return;
      }
      ...
      if (!mountedRef.current || generation !== revisionRef.current) return;
      const runtimeError = error instanceof WeekFourBossRuntimeError ? error : null;
      ...
      const infrastructureAttempt = await attemptContextWrite(() => context.saveWeekFourBossInfrastructureFailure(infrastructureInput));
      if (!mountedRef.current || generation !== revisionRef.current) return;
  ...
  const observe = async () => {
    ...
    setLock(true, 'session-pending');
    const generation = revisionRef.current;
    const observationAttempt = await attemptContextWrite(() => context.saveWeekFourBossObservation());
    if (!mountedRef.current || generation !== revisionRef.current) return;
  ...
  const retryRejectedWrite = (request: RejectedWrite): Promise<WriteAttempt> => {
    if (request.kind === 'draft') return attemptContextWrite(() => context.saveWeekFourBossDraft(request.code));
    if (request.kind === 'run') return attemptContextWrite(() => context.saveWeekFourBossRun(request.input));
    if (request.kind === 'validation') return attemptContextWrite(() => context.saveWeekFourBossValidationFailure());
    if (request.kind === 'infrastructure') return attemptContextWrite(() => context.saveWeekFourBossInfrastructureFailure(request.input));
    if (request.kind === 'observation') return attemptContextWrite(() => context.saveWeekFourBossObservation());
    if (request.kind === 'completion') return attemptContextWrite(() => context.completeWeekFourBoss(request.input));
  ...
  const retry = async () => {
    ...
    try {
      ...
      setMessage(saved.lastRun ? feedbackFor(saved.lastRun) : '核验运行结果已保存。');
      finish();
    } finally {
  ...
  const reloadExternal = () => {
    ...
    pendingRunRef.current = null;
    pendingInfrastructureRef.current = null;
    const externalSession = external.sessions['w4-m5'];
    const next = externalSession ?? createMissionSession('w4-m5', new Date().toISOString());
    draftPersistedRef.current = Boolean(externalSession);
    sessionRef.current = next;
    setSession(next);
    setEditorCode(next.pythonCode);
    setObservationVisible(false);
    setFormalCompleted(external.missionCompletionEvidence['w4-m5']?.kind === 'formal-v3');
    clearRecovery();
    if (externalSession) {
      setOperation('idle');
    ...
    setLock(true, 'session-pending');
    void (async () => {
      const attempt = await attemptContextWrite(() => context.saveWeekFourBossDraft(next.pythonCode));
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (!attempt.ok) {
  ...
  void onComplete;
  return (
    <section className="week-four-boss-experience" data-reduced-motion={reducedMotion} data-muted={muted} data-assets-ready={assetsReady}>
      <header>
        <p className="eyebrow">W4-M5 身份核验</p>
        <h2>白虎岭核验站</h2>
        <p>同一段程序，连续核验两组公开卡片。外形相同，身份也一定相同吗？</p>
      </header>
      <div className="week-four-boss-layout">
        <LazySectionBoundary label="身份核验场景">
          <Suspense fallback={<p role="status">循环场景加载中…</p>}>
            <Scene state={sceneState} cards={WEEK_FOUR_BOSS_CARDS} events={displayCurrent ? session.lastCanonicalTrace : []} muted={muted} reducedMotion={reducedMotion} showCanonEpilogue={formalCompleted} onAssetsReady={() => {
        ...
        <section className="week-four-boss-preview-feedback" aria-live="polite">
          <p ref={feedbackRef} tabIndex={-1} role={pending ? 'alert' : 'status'} aria-label="核验运行结果">{message}</p>
          <p role="status" aria-label={runtimeReady ? 'Python 运行环境已准备' : 'Python 运行环境加载中'}>{runtimeReady ? 'Python 运行环境已准备' : 'Python 运行环境加载中'}</p>
          <button type="button" className="button button-primary" disabled={blocked} onClick={() => void run()}>运行两轮核验</button>
...
export default WeekFourBossExperience;
