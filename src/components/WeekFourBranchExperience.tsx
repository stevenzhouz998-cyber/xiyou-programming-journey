...
import { WEEK_FOUR_BRANCH_CARDS, type WeekFourBranchRunResult, type WeekFourBranchTraceItem } from '../engine/weekFourBranchContract';
import { parseWeekFourBranchPython } from '../engine/weekFourBranchPythonGrammar';
import { createWeekFourBranchPythonRuntime, WeekFourBranchRuntimeError, type WeekFourBranchPythonRuntime } from '../engine/weekFourBranchPythonRunner';
import { createMissionSession } from '../progress/session';
import type { CoordinatedSaveResult } from '../progress/storageCoordinator';
import type { WeekFourBranchMissionSession } from '../progress/weekFourBranchSession';
import type { WeekFourVariableWorkV1 } from '../progress/types';
import { downloadTextFile } from '../utils/download';
import { LazySectionBoundary } from './LazySectionBoundary';
import type { WeekFourBranchPythonEditorHandle, WeekFourBranchPythonEditorProps } from './WeekFourBranchPythonEditor';
import type { WeekFourBranchSceneProps } from './WeekFourBranchScene';
import { WeekFourVariableWorkReview } from './WeekFourVariableWorkReview';
import './WeekFourBranchExperience.css';

const loadWeekFourBranchPythonEditor = () => import('./WeekFourBranchPythonEditor').then((module) => ({ default: module.WeekFourBranchPythonEditor }));
const loadWeekFourBranchScene = () => import('./WeekFourBranchScene').then((module) => ({ default: module.WeekFourBranchScene }));

export interface WeekFourBranchExperienceProps {
  reducedMotion: boolean;
  muted: boolean;
  locked?: boolean;
  work?: WeekFourVariableWorkV1;
  onComplete(evidence: { stars: 1 | 2 | 3; hintsUsed: number }): void | boolean | Promise<boolean>;
  runtimeFactory?: () => WeekFourBranchPythonRuntime;
...
type BranchRunInput = { canonicalTrace: WeekFourBranchTraceItem[]; workerTrace: WeekFourBranchTraceItem[]; run: WeekFourBranchRunResult };
type RejectedWrite =
...
function feedbackFor(run: WeekFourBranchRunResult): string {
  const canon = run.cardResults[0];
  const practice = run.cardResults[1];
  if (canon.result === 'branch-conflict' && practice.result === 'branch-missing') {
    return '原著卡路线冲突，练习卡没有路线；这两张卡的事实已保存。';
  }
  if (run.failureSnapshots.some((snapshot) => snapshot.result === 'branch-conflict')) {
    return '两条路线同时发生；这个分支事实已保存。';
  }
  if (run.failureSnapshots.some((snapshot) => snapshot.result === 'branch-missing')) {
    return '有一张卡没有走到任何路线；这个分支事实已保存。';
  }
  return '两张卡都只走了一条路线。';
}

export function WeekFourBranchExperience({
  reducedMotion,
  muted,
  locked = false,
  work,
  onComplete,
  runtimeFactory = createWeekFourBranchPythonRuntime,
  onSessionPersistenceActiveChange,
  onInteractionLockChange,
}: WeekFourBranchExperienceProps) {
  const context = useProgress();
  const persistedInitial = context.progress.sessions['w4-m3'];
  const initial = useMemo<WeekFourBranchMissionSession>(() => persistedInitial ?? createMissionSession('w4-m3', new Date().toISOString()), []);
  ...
  const [message, setMessage] = useState(persistedInitial ? '先运行两张公开卡，看看每张卡实际走了几条路线。' : '正在保存默认 Python 草稿…');
  ...
  const [formalCompleted, setFormalCompleted] = useState(context.progress.missionCompletionEvidence['w4-m3']?.kind === 'formal-v3');
  ...
  const runtimeRef = useRef<WeekFourBranchPythonRuntime | null>(null);
  const editorRef = useRef<WeekFourBranchPythonEditorHandle | null>(null);
  ...
  const pendingRunRef = useRef<{ canonicalTrace: WeekFourBranchTraceItem[]; workerTrace: WeekFourBranchTraceItem[]; run: WeekFourBranchRunResult } | null>(null);
  ...
  const Editor = useMemo(() => lazy(loadWeekFourBranchPythonEditor) as React.ComponentType<WeekFourBranchPythonEditorProps & React.RefAttributes<WeekFourBranchPythonEditorHandle>>, [editorGeneration]);
  const Scene = useMemo(() => lazy(loadWeekFourBranchScene) as React.ComponentType<WeekFourBranchSceneProps>, []);
  ...
  const accept = (result: CoordinatedSaveResult, expectedGeneration: number): WeekFourBranchMissionSession | null => {
    if (!mountedRef.current || expectedGeneration !== revisionRef.current) return null;
    if (result.status !== 'saved') return null;
    const next = result.progress.sessions['w4-m3'];
    if (!next) return null;
  ...
  const finishFormalCompletion = async (result: CoordinatedSaveResult, generation: number) => {
    ...
    const savedMission = result.progress.missions?.['w4-m3'];
    const proof = result.progress.missionCompletionEvidence?.['w4-m3'];
    const savedWork = result.progress.works?.['w4-m3-branch-structure-record'];
    ...
    if (result.status !== 'saved' || proof?.kind !== 'formal-v3' || savedWork?.kind !== 'python-branch-structure-v1') {
      completionActiveRef.current = false;
  ...
  const completeSolved = async () => {
    ...
    const attempt = await attemptContextWrite(() => context.completeWeekFourBranch(input));
    if (!mountedRef.current || generation !== revisionRef.current) return;
    if (!attempt.ok) {
  ...
  useEffect(() => {
    ...
    setLock(true, 'session-pending');
    void (async () => {
      const attempt = await attemptContextWrite(() => context.saveWeekFourBranchDraft(initial.pythonCode));
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (!attempt.ok) {
      ...
      setMessage('默认 Python 草稿已安全保存，可以运行分支。');
      finish();
    })();
  }, []);
  ...
  const edit = async (code: string) => {
    ...
    setLock(true, 'session-pending');
    const attempt = await attemptContextWrite(() => context.saveWeekFourBranchDraft(code));
    if (!mountedRef.current || generation !== revisionRef.current) return;
    if (!attempt.ok) {
    ...
    clearRecovery();
    setMessage('新的 Python 草稿已保存；旧的分支结果已失效。');
    finish();
  };
  ...
  const replay = async () => {
    ...
    try {
      ...
      const mission = context.progress.missions['w4-m3'];
      const prerequisite = context.progress.missionCompletionEvidence['w4-m2'];
      const proof = context.progress.missionCompletionEvidence['w4-m3'];
      const savedWork = context.progress.works['w4-m3-branch-structure-record'];
      ...
      if (!saved.lastRun?.completed
        ...
        || proof.workId !== 'w4-m3-branch-structure-record'
        || !savedWork
        || savedWork.kind !== 'python-branch-structure-v1'
        || savedWork.workId !== 'w4-m3-branch-structure-record'
        || savedWork.missionId !== 'w4-m3'
        || saved.kind !== 'python-branch-structure-v1'
      ...
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (error instanceof WeekFourBranchRuntimeError && !['validation', 'busy'].includes(error.code)) setRuntimeReady(false);
  ...
  const run = async () => {
    ...
    const draftAttempt = await attemptContextWrite(() => context.saveWeekFourBranchDraft(candidate));
    if (!mountedRef.current || generation !== revisionRef.current) return;
    if (!draftAttempt.ok) {
    ...
    setMessage('正在运行刚刚保存的 Python 分支…');
    let parsed;
    try { parsed = parseWeekFourBranchPython(saved.pythonCode); }
    catch {
    ...
    if ('state' in parsed) {
      const validationAttempt = await attemptContextWrite(() => context.saveWeekFourBranchValidationFailure());
      if (!mountedRef.current || generation !== revisionRef.current) return;
      ...
      clearRecovery();
      if (parsed.line === 4) editorRef.current?.focusAction();
    ...
    try {
      ...
      const runAttempt = await attemptContextWrite(() => context.saveWeekFourBranchRun(input));
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (!runAttempt.ok) {
        pendingRunRef.current = input;
        recoverRejected({ kind: 'run', stage: 'run', input, message: '分支运行结果保存异常，场景没有发布这次结果。' });
        return;
      }
      ...
      if (!savedRun) {
        pendingRunRef.current = input;
        recover('run', runResult.status === 'conflict' ? 'conflict' : 'unsaved', '分支运行结果尚未保存，当前场景不会把它当作事实。');
        return;
      }
      ...
      if (savedRun.lastRun?.completed) {
        if (!assetsReadyRef.current) {
          setMessage('分支已证明，等待场景资源就绪后再封存作品与证明。');
          finish();
          return;
        }
      ...
      if (!mountedRef.current || generation !== revisionRef.current) return;
      const runtimeError = error instanceof WeekFourBranchRuntimeError ? error : null;
      ...
      const infrastructureAttempt = await attemptContextWrite(() => context.saveWeekFourBranchInfrastructureFailure(infrastructureInput));
      if (!mountedRef.current || generation !== revisionRef.current) return;
  ...
  const observe = async () => {
    ...
    setLock(true, 'session-pending');
    const generation = revisionRef.current;
    const observationAttempt = await attemptContextWrite(() => context.saveWeekFourBranchObservation());
    if (!mountedRef.current || generation !== revisionRef.current) return;
    ...
    setMessage('火眼金睛只展示已保存的实际路线。');
    finish();
  };

  const retryRejectedWrite = (request: RejectedWrite): Promise<WriteAttempt> => {
    if (request.kind === 'draft') return attemptContextWrite(() => context.saveWeekFourBranchDraft(request.code));
    if (request.kind === 'run') return attemptContextWrite(() => context.saveWeekFourBranchRun(request.input));
    if (request.kind === 'validation') return attemptContextWrite(() => context.saveWeekFourBranchValidationFailure());
    if (request.kind === 'infrastructure') return attemptContextWrite(() => context.saveWeekFourBranchInfrastructureFailure(request.input));
    if (request.kind === 'observation') return attemptContextWrite(() => context.saveWeekFourBranchObservation());
    if (request.kind === 'completion') return attemptContextWrite(() => context.completeWeekFourBranch(request.input));
  ...
  const retry = async () => {
    ...
    try {
      ...
      const saved = accept(result, generation);
      if (!saved) {
        recover(stage, 'unsaved', '重试后仍未找到已保存的分支记录。');
        return;
      }
      clearRecovery();
      if (stage === 'draft') {
      ...
      setMessage(saved.lastRun ? feedbackFor(saved.lastRun) : '分支运行结果已保存。');
      finish();
    } finally {
  ...
  const reloadExternal = () => {
    ...
    pendingRunRef.current = null;
    pendingInfrastructureRef.current = null;
    const externalSession = external.sessions['w4-m3'];
    const next = externalSession ?? createMissionSession('w4-m3', new Date().toISOString());
    draftPersistedRef.current = Boolean(externalSession);
    sessionRef.current = next;
    setSession(next);
    setEditorCode(next.pythonCode);
    setObservationVisible(false);
    setFormalCompleted(external.missionCompletionEvidence['w4-m3']?.kind === 'formal-v3');
    clearRecovery();
    if (externalSession) {
      setOperation('idle');
      setMessage('已载入其他标签页保存的分支进度。');
      setLock(false, 'idle');
      return;
    }
    const generation = revisionRef.current;
    ...
    setLock(true, 'session-pending');
    void (async () => {
      const attempt = await attemptContextWrite(() => context.saveWeekFourBranchDraft(next.pythonCode));
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (!attempt.ok) {
  ...
  void onComplete;
  return (
    <section className="week-four-branch-experience" data-reduced-motion={reducedMotion} data-muted={muted} data-assets-ready={assetsReady}>
      <header>
        <p className="eyebrow">W4-M3 分支归位</p>
        <h2>同样的外形，一次只走一条路线</h2>
        <p>运行两张公开卡，核对每张卡实际执行的路线。</p>
      </header>
      <div className="week-four-branch-layout">
        <LazySectionBoundary label="分支归位场景">
          <Suspense fallback={<p role="status">分支场景加载中…</p>}>
            <Scene state={sceneState} cards={WEEK_FOUR_BRANCH_CARDS} events={displayCurrent ? session.lastCanonicalTrace : []} muted={muted} reducedMotion={reducedMotion} showCanonEpilogue={formalCompleted} onAssetsReady={() => {
        ...
        <WeekFourVariableWorkReview work={work} />
        <LazySectionBoundary label="Python 分支编辑器">
        ...
        <section className="week-four-branch-preview-feedback" aria-live="polite">
          <p ref={feedbackRef} tabIndex={-1} role={pending ? 'alert' : 'status'} aria-label="分支运行结果">{message}</p>
          <p role="status" aria-label={runtimeReady ? 'Python 运行环境已准备' : 'Python 运行环境加载中'}>{runtimeReady ? 'Python 运行环境已准备' : 'Python 运行环境加载中'}</p>
          <button type="button" className="button button-primary" disabled={blocked} onClick={() => void run()}>运行分支</button>
          {session.failureSnapshot && displayCurrent ? <button type="button" className="button button-ghost" disabled={blocked} onClick={() => void observe()}>火眼金睛：观察实际路线</button> : null}
      ...
      {observationVisible && session.failureSnapshot ? <aside role="status" aria-label="已保存的实际路线">
        <h3>火眼金睛：已保存的实际路线</h3>
        <p>这张卡实际执行了：{session.failureSnapshot.actualActions.length === 0 ? '没有路线' : session.failureSnapshot.actualActions.map((action) => action === 'keep-observing' ? '继续观察路线' : '礼貌帮助路线').join('、')}。</p>
      </aside> : null}
...
export default WeekFourBranchExperience;
