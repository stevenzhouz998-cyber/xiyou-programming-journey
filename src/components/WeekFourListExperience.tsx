...
import { WEEK_FOUR_LIST_CARDS, type WeekFourListRunResult, type WeekFourListTraceItem } from '../engine/weekFourListContract';
import { parseWeekFourListPython } from '../engine/weekFourListPythonGrammar';
import { createWeekFourListPythonRuntime, WeekFourListRuntimeError, type WeekFourListPythonRuntime } from '../engine/weekFourListPythonRunner';
import { createMissionSession } from '../progress/session';
import type { CoordinatedSaveResult } from '../progress/storageCoordinator';
import type { WeekFourListMissionSession } from '../progress/weekFourListSession';
import { downloadTextFile } from '../utils/download';
import { LazySectionBoundary } from './LazySectionBoundary';
import type { WeekFourListPythonEditorHandle, WeekFourListPythonEditorProps } from './WeekFourListPythonEditor';
import type { WeekFourListSceneProps } from './WeekFourListScene';

import './WeekFourListExperience.css';

const loadWeekFourListPythonEditor = () => import('./WeekFourListPythonEditor').then((module) => ({ default: module.WeekFourListPythonEditor }));
const loadWeekFourListScene = () => import('./WeekFourListScene').then((module) => ({ default: module.WeekFourListScene }));

export interface WeekFourListExperienceProps {
  reducedMotion: boolean;
  muted: boolean;
  locked?: boolean;
  onComplete(evidence: { stars: 1 | 2 | 3; hintsUsed: number }): void | boolean | Promise<boolean>;
  runtimeFactory?: () => WeekFourListPythonRuntime;
...
type BranchRunInput = { canonicalTrace: WeekFourListTraceItem[]; workerTrace: WeekFourListTraceItem[]; run: WeekFourListRunResult };
type RejectedWrite =
...
function feedbackFor(run: WeekFourListRunResult): string {
  if (run.state === 'list-order-conflict') return '列表的顺序或数量与公开原著不符；逐轮事实已保存。';
  if (run.state === 'loop-value-conflict') return '有的轮次记录了固定文字，没有记录当前 item；逐轮事实已保存。';
  return '三次变化都按原著顺序逐项记录。';
}

export function WeekFourListExperience({
  reducedMotion,
  muted,
  locked = false,
  onComplete,
  runtimeFactory = createWeekFourListPythonRuntime,
  onSessionPersistenceActiveChange,
  onInteractionLockChange,
}: WeekFourListExperienceProps) {
  const context = useProgress();
  const persistedInitial = context.progress.sessions['w4-m4'];
  const initial = useMemo<WeekFourListMissionSession>(() => persistedInitial ?? createMissionSession('w4-m4', new Date().toISOString()), []);
  ...
  const [message, setMessage] = useState(persistedInitial ? (initial.lastRun?.completed ? '观察册和正式证明已保存；可以重新运行核验。' : initial.lastRun ? feedbackFor(initial.lastRun) : '先运行观察程序，对照每一轮取出的 item 和实际记录值。') : '正在保存默认 Python 草稿…');
  ...
  const [formalCompleted, setFormalCompleted] = useState(context.progress.missionCompletionEvidence['w4-m4']?.kind === 'formal-v3');
  ...
  const runtimeRef = useRef<WeekFourListPythonRuntime | null>(null);
  const editorRef = useRef<WeekFourListPythonEditorHandle | null>(null);
  ...
  const pendingRunRef = useRef<{ canonicalTrace: WeekFourListTraceItem[]; workerTrace: WeekFourListTraceItem[]; run: WeekFourListRunResult } | null>(null);
  ...
  const Editor = useMemo(() => lazy(loadWeekFourListPythonEditor) as React.ComponentType<WeekFourListPythonEditorProps & React.RefAttributes<WeekFourListPythonEditorHandle>>, [editorGeneration]);
  const Scene = useMemo(() => lazy(loadWeekFourListScene) as React.ComponentType<WeekFourListSceneProps>, []);
  ...
  const accept = (result: CoordinatedSaveResult, expectedGeneration: number): WeekFourListMissionSession | null => {
    if (!mountedRef.current || expectedGeneration !== revisionRef.current) return null;
    if (result.status !== 'saved') return null;
    const next = result.progress.sessions['w4-m4'];
    if (!next) return null;
  ...
  const finishFormalCompletion = async (result: CoordinatedSaveResult, generation: number) => {
    ...
    const savedMission = result.progress.missions?.['w4-m4'];
    const proof = result.progress.missionCompletionEvidence?.['w4-m4'];
    const savedWork = result.progress.works?.['w4-m4-list-loop-record'];
    ...
    if (result.status !== 'saved' || proof?.kind !== 'formal-v3' || savedWork?.kind !== 'python-list-loop-v1') {
      completionActiveRef.current = false;
  ...
  const completeSolved = async () => {
    ...
    const attempt = await attemptContextWrite(() => context.completeWeekFourList(input));
    if (!mountedRef.current || generation !== revisionRef.current) return;
    if (!attempt.ok) {
  ...
  useEffect(() => {
    ...
    setLock(true, 'session-pending');
    void (async () => {
      const attempt = await attemptContextWrite(() => context.saveWeekFourListDraft(initial.pythonCode));
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (!attempt.ok) {
      ...
      setMessage('默认 Python 草稿已安全保存，可以运行循环。');
      finish();
    })();
  }, []);
  ...
  const edit = async (code: string) => {
    ...
    setLock(true, 'session-pending');
    const attempt = await attemptContextWrite(() => context.saveWeekFourListDraft(code));
    if (!mountedRef.current || generation !== revisionRef.current) return;
    if (!attempt.ok) {
  ...
  const replay = async () => {
    ...
    try {
      ...
      const mission = context.progress.missions['w4-m4'];
      const prerequisite = context.progress.missionCompletionEvidence['w4-m3'];
      const proof = context.progress.missionCompletionEvidence['w4-m4'];
      const savedWork = context.progress.works['w4-m4-list-loop-record'];
      ...
      if (!saved.lastRun?.completed
        ...
        || proof.workId !== 'w4-m4-list-loop-record'
        || !savedWork
        || savedWork.kind !== 'python-list-loop-v1'
        || savedWork.workId !== 'w4-m4-list-loop-record'
        || savedWork.missionId !== 'w4-m4'
        || saved.kind !== 'python-list-loop-v1'
      ...
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (error instanceof WeekFourListRuntimeError && !['validation', 'busy'].includes(error.code)) setRuntimeReady(false);
  ...
  const run = async () => {
    ...
    const draftAttempt = await attemptContextWrite(() => context.saveWeekFourListDraft(candidate));
    if (!mountedRef.current || generation !== revisionRef.current) return;
    if (!draftAttempt.ok) {
    ...
    let parsed;
    try { parsed = parseWeekFourListPython(saved.pythonCode); }
    catch {
    ...
    if ('state' in parsed) {
      const validationAttempt = await attemptContextWrite(() => context.saveWeekFourListValidationFailure());
      if (!mountedRef.current || generation !== revisionRef.current) return;
    ...
    try {
      ...
      const runAttempt = await attemptContextWrite(() => context.saveWeekFourListRun(input));
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (!runAttempt.ok) {
        pendingRunRef.current = input;
        recoverRejected({ kind: 'run', stage: 'run', input, message: '循环运行结果保存异常，场景没有发布这次结果。' });
        return;
      }
      ...
      if (!savedRun) {
        pendingRunRef.current = input;
        recover('run', runResult.status === 'conflict' ? 'conflict' : 'unsaved', '循环运行结果尚未保存，当前场景不会把它当作事实。');
        return;
      }
      ...
      if (!mountedRef.current || generation !== revisionRef.current) return;
      const runtimeError = error instanceof WeekFourListRuntimeError ? error : null;
      ...
      const infrastructureAttempt = await attemptContextWrite(() => context.saveWeekFourListInfrastructureFailure(infrastructureInput));
      if (!mountedRef.current || generation !== revisionRef.current) return;
  ...
  const observe = async () => {
    ...
    setLock(true, 'session-pending');
    const generation = revisionRef.current;
    const observationAttempt = await attemptContextWrite(() => context.saveWeekFourListObservation());
    if (!mountedRef.current || generation !== revisionRef.current) return;
  ...
  const retryRejectedWrite = (request: RejectedWrite): Promise<WriteAttempt> => {
    if (request.kind === 'draft') return attemptContextWrite(() => context.saveWeekFourListDraft(request.code));
    if (request.kind === 'run') return attemptContextWrite(() => context.saveWeekFourListRun(request.input));
    if (request.kind === 'validation') return attemptContextWrite(() => context.saveWeekFourListValidationFailure());
    if (request.kind === 'infrastructure') return attemptContextWrite(() => context.saveWeekFourListInfrastructureFailure(request.input));
    if (request.kind === 'observation') return attemptContextWrite(() => context.saveWeekFourListObservation());
    if (request.kind === 'completion') return attemptContextWrite(() => context.completeWeekFourList(request.input));
  ...
  const retry = async () => {
    ...
    try {
      ...
      setMessage(saved.lastRun ? feedbackFor(saved.lastRun) : '循环运行结果已保存。');
      finish();
    } finally {
  ...
  const reloadExternal = () => {
    ...
    pendingRunRef.current = null;
    pendingInfrastructureRef.current = null;
    const externalSession = external.sessions['w4-m4'];
    const next = externalSession ?? createMissionSession('w4-m4', new Date().toISOString());
    draftPersistedRef.current = Boolean(externalSession);
    sessionRef.current = next;
    setSession(next);
    setEditorCode(next.pythonCode);
    setObservationVisible(false);
    setFormalCompleted(external.missionCompletionEvidence['w4-m4']?.kind === 'formal-v3');
    clearRecovery();
    if (externalSession) {
      setOperation('idle');
    ...
    setLock(true, 'session-pending');
    void (async () => {
      const attempt = await attemptContextWrite(() => context.saveWeekFourListDraft(next.pythonCode));
      if (!mountedRef.current || generation !== revisionRef.current) return;
      if (!attempt.ok) {
  ...
  void onComplete;
  return (
    <section className="week-four-list-experience" data-reduced-motion={reducedMotion} data-muted={muted} data-assets-ready={assetsReady}>
      <header>
        <p className="eyebrow">W4-M4 逐项观察</p>
        <h2>三次变化，逐项留下观察记录</h2>
        <p>调整列表顺序和每轮记录的值，用一个循环整理三次变化。</p>
      </header>
      <div className="week-four-list-layout">
        <LazySectionBoundary label="逐项观察场景">
          <Suspense fallback={<p role="status">循环场景加载中…</p>}>
            <Scene state={sceneState} cards={WEEK_FOUR_LIST_CARDS} events={displayCurrent ? session.lastCanonicalTrace : []} muted={muted} reducedMotion={reducedMotion} showCanonEpilogue={formalCompleted} onAssetsReady={() => {
        ...
        <section className="week-four-list-preview-feedback" aria-live="polite">
          <p ref={feedbackRef} tabIndex={-1} role={pending ? 'alert' : 'status'} aria-label="循环运行结果">{message}</p>
          <p role="status" aria-label={runtimeReady ? 'Python 运行环境已准备' : 'Python 运行环境加载中'}>{runtimeReady ? 'Python 运行环境已准备' : 'Python 运行环境加载中'}</p>
          <button type="button" className="button button-primary" disabled={blocked} onClick={() => void run()}>运行循环</button>
...
export default WeekFourListExperience;
