...
import { hasValidFormalWeekSixArchiveCompletion,isMissionUnlocked } from '../progress/progress';
...
export function HomePage({ onOpenEquipment }: { onOpenEquipment: () => void }) {
  ...
  const archiveComplete=hasValidFormalWeekSixArchiveCompletion(progress);
  const nextMission = archiveComplete?allMissionOutlines[allMissionOutlines.length-1]!:allMissionOutlines.find((mission) => isMissionUnlocked(progress, mission.id) && !progress.missions[mission.id]) ?? allMissionOutlines[0];
