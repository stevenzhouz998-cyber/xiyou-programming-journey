...
export function RuyiStaffScene({ events, replayToken, reducedMotion, muted, onPlaybackComplete }: Props) {
  ...
  const apply = (owner: symbol, scene: Phaser.Scene, nodes: Nodes, event: RuyiStaffBattleEvent) => {
    ...
    if (event.type === 'state-changed') {
      ...
      if (event.state === 'weights-inspected') {
        hideWeapons(nodes)
        if (nodes.sabre) { showSabre(nodes.sabre, 90, 135); nodes.sabre.setX(scene.scale.width * .43) }
    ...
    if (event.type === 'instruction-rejected') {
      ...
      if (event.opcode === 'choose_sabre') {
        hideWeapons(nodes); setSelected('sabre')
        const show = () => {
          if (ownerRef.current !== owner) return
        ...
        if (scene.textures.exists('sabre')) show()
          ...
          scene.load.once('complete', () => { if (generation === generationRef.current) show() })
  ...
  const retry = () => { generationRef.current += 1; activeRef.current = false; setLoadError(null); setState('awaiting-inspection'); setMessages([]); setAttempt((value) => value + 1) }
