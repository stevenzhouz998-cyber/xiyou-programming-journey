...
const loadWeekFiveMonksExperience = () => import('./WeekFiveMonksExperience').then((module) => ({ default: module.WeekFiveMonksExperience }));
...
const loadWeekFiveStoryOrchestrationExperience = () => import('./WeekFiveStoryOrchestrationExperience').then((module) => ({ default: module.WeekFiveStoryOrchestrationExperience }));
...
const loadWeekSixClassificationExperience=()=>import('./WeekSixClassificationExperience').then((module)=>({default:module.WeekSixClassificationExperience}));
...
const loadWeekSixFactCheckExperience=()=>import('./WeekSixFactCheckExperience').then(module=>({default:module.WeekSixFactCheckExperience}));
...
export function WeekThreeBossRouteBoundary({ loader = loadWeekThreeBossExperience, reloadPage, ...props }: WeekThreeBossExperienceProps & { loader?: () => Promise<{ default: ComponentType<WeekThreeBossExperienceProps> }>; reloadPage?: () => void }) {
  const [retryGeneration, setRetryGeneration] = useState(0);
  const Experience = useMemo(() => lazy(loader), [loader, retryGeneration]);
...
export function WeekFiveMonksRouteBoundary({ loader = loadWeekFiveMonksExperience, reloadPage: _reloadPage, ...props }: WeekFiveMonksExperienceProps & { loader?: () => Promise<{ default: ComponentType<WeekFiveMonksExperienceProps> }>; reloadPage?: () => void }) {
  const [retryGeneration, setRetryGeneration] = useState(0);
  const selectedLoader = loader === loadWeekFiveMonksExperience && retryGeneration > 0
    ? loadWeekFiveMonksExperienceRetry
    : loader;
  const Experience = useMemo(() => lazy(selectedLoader), [selectedLoader, retryGeneration]);
  const retry = () => setRetryGeneration((generation) => generation + 1);
  return <LazySectionBoundary key={retryGeneration} label="逐项观察体验" reloadPage={retry}><Suspense fallback={<p className="mission-tools-loading" role="status">逐项观察体验加载中，请稍候……</p>}><Experience {...props} /></Suspense></LazySectionBoundary>;
...
export function WeekFiveStoryOrchestrationRouteBoundary({ loader = loadWeekFiveStoryOrchestrationExperience, reloadPage: _reloadPage, ...props }: WeekFiveStoryOrchestrationExperienceProps & { loader?: () => Promise<{ default: ComponentType<WeekFiveStoryOrchestrationExperienceProps> }>; reloadPage?: () => void }) {
  const [retryGeneration, setRetryGeneration] = useState(0);
  const selectedLoader = loader === loadWeekFiveStoryOrchestrationExperience && retryGeneration > 0 ? loadWeekFiveStoryOrchestrationExperienceRetry : loader;
  const Experience = useMemo(() => lazy(selectedLoader), [selectedLoader, retryGeneration]);
...
export function MissionPageForId({
  ...
  const revealPersistedWeekFiveMonksCompletion = async (
    earnedStars: number,
    completionHints: number,
  ): Promise<boolean> => {
    if (mission.id !== 'w5-m1' || successRef.current || completionSaveRef.current !== null) return false;
    const request: CompletionSave = {
      requestId: ++requestGenerationRef.current,
      stars: earnedStars,
      hintsUsed: completionHints,
  ...
  const revealPersistedWeekFiveStoryOrchestrationCompletion = async (earnedStars: number, completionHints: number): Promise<boolean> => {
    if (mission.id !== 'w5-m5' || successRef.current || completionSaveRef.current !== null) return false;
    const request: CompletionSave = { requestId: ++requestGenerationRef.current, stars: earnedStars, hintsUsed: completionHints, status: 'pending' };
    onCompletionPersistenceActiveChange(true); completionSaveRef.current = request; return revealSuccess(request, earnedStars);
  };
  ...
  const hintsLocked = isExecutableMissionId(mission.id)
    && (battleInteractionLocked || completionSave !== null || (mission.id === 'w6-m1' && progress.missionCompletionEvidence['w6-m1']?.kind === 'formal-v3') || (mission.id === 'w6-m2' && progress.missionCompletionEvidence['w6-m2']?.kind === 'formal-v3') || (mission.id === 'w6-m3' && progress.missionCompletionEvidence['w6-m3']?.kind === 'formal-v3') || (mission.id === 'w6-m4' && progress.missionCompletionEvidence['w6-m4']?.kind === 'formal-v3'));
  const hintLockReason =
    (mission.id === 'w6-m1' && progress.missionCompletionEvidence['w6-m1']?.kind === 'formal-v3') || (mission.id === 'w6-m2' && progress.missionCompletionEvidence['w6-m2']?.kind === 'formal-v3') || (mission.id === 'w6-m3' && progress.missionCompletionEvidence['w6-m3']?.kind === 'formal-v3') || (mission.id === 'w6-m4' && progress.missionCompletionEvidence['w6-m4']?.kind === 'formal-v3')
      ? "正式作品已保存，回放不会再改写提示记录。"
        ...
        <div className="mission-layout">
          ...
          <section className="play-column">
            ...
            {mission.id === 'w1-m1' ? (
              ...
              <WeekFiveMonksRouteBoundary
                loader={weekFiveMonksLoader}
                runtimeFactory={weekFiveMonksRuntimeFactory}
                reducedMotion={reducedMotion}
                muted={progress.settings.muted}
                locked={completionSave !== null}
              ...
              <WeekFiveStoryOrchestrationRouteBoundary
                loader={weekFiveStoryOrchestrationLoader}
                runtimeFactory={weekFiveStoryOrchestrationRuntimeFactory}
                reducedMotion={reducedMotion}
                muted={progress.settings.muted}
                locked={completionSave !== null}
                onComplete={({ stars: earnedStars, hintsUsed: used }) => revealPersistedWeekFiveStoryOrchestrationCompletion(earnedStars, used)}
                onSessionPersistenceActiveChange={onCompletionPersistenceActiveChange}
                onInteractionLockChange={setBattleInteractionLocked}
              />
            ) : mission.id==='w6-m1' ? (
