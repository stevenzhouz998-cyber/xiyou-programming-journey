...
const loadAdvancedWeekOneExperience = () => import('./AdvancedWeekOneExperience').then((module) => ({ default: module.AdvancedWeekOneExperience }));
...
export function AdvancedWeekOneRouteBoundary({
  ...
  loader?: () => Promise<{ default: ComponentType<AdvancedWeekOneExperienceProps> }>;
  reloadPage?: () => void;
}) {
  const Experience = useMemo(() => lazy(loader), [loader]);
  return <LazySectionBoundary label="幽冥与总试炼任务" reloadPage={reloadPage}><Suspense fallback={<p className="mission-tools-loading" role="status">正式任务加载中，请稍候……</p>}><Experience {...props} /></Suspense></LazySectionBoundary>;
}
...
function MissionPageForId({ id, mission, reducedMotion, onGlobalModalOpenChange, onCompletionPersistenceActiveChange }: MissionPageProps & { id: string; mission: MissionSpec | FormalMissionSpec | undefined }) {
  ...
  const renderLegacyMissionTools = () => {
    if (legacySequence === null) throw new Error(`正式任务 ${mission.id} 不能进入兼容任务工具`);
    ...
    return <LazySectionBoundary label="兼容任务工具"><Suspense fallback={<p className="mission-tools-loading" role="status">兼容任务工具加载中，请稍候……</p>}><MissionTools missionId={mission.id} mode={mission.mode} sceneProps={{ activeStep, reducedMotion }} toolProps={toolProps} /></Suspense></LazySectionBoundary>;
  };
  const hintsLocked = (mission.id === 'w1-m2' || mission.id === 'w1-m3' || mission.id === 'w1-m4' || mission.id === 'w1-m5') && (battleInteractionLocked || completionSave !== null);
