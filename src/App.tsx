...
function MissionPageContent({ reducedMotion, id }: { reducedMotion: boolean; id: string }) {
  ...
  const { progress, complete, recordMissionHint } = useProgress();
  ...
  const pass = (earnedStars: number, completionHints = hintsUsed) => {
    setStars(earnedStars); setSuccess(true); complete(mission.id, { stars: earnedStars, hintsUsed: completionHints }); playAudio(assetUrl(mission.isBoss ? '/assets/audio/boss.m4a' : '/assets/audio/success.m4a'), progress.settings.muted);
  };
...
function AppRoutes() {
  ...
  return <GlobalModalIsolationContext.Provider value={setGlobalModalOpen}><div className="app-shell" data-testid="app-shell" data-reduced-motion={String(effectiveReducedMotion)}>
    <RecoveryNotice loadStatus={loadStatus} persistence={persistence} loadError={loadError} corruptError={corruptError} saveError={saveError} hasCorruptDownload={corruptDownload !== null} onRetry={retrySave} />
