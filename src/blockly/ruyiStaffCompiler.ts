...
import { isRuyiBlockType, RUYI_BLOCK_OPCODE } from './ruyiStaffBlocks'

export type RuyiCompileDiagnosticCode =
  | 'unknown-block'
  | 'invalid-connection'
  | 'multiple-top-level'
  | 'empty-workspace'

export type RuyiCompileResult =
  | { ok: true; trace: RuyiStaffInstruction[] }
  | {
      ok: false
      trace: []
      diagnostics: Array<{
        code: RuyiCompileDiagnosticCode
...
function hasCanonicalConnections(block: Block): boolean {
  ...
  } else if (block.getPreviousBlock() !== null) return false
  ...
  } else if (block.getNextBlock() !== null) return false

  return true
}

export function compileRuyiStaffWorkspace(workspace: Workspace): RuyiCompileResult {
  const blocks = workspace.getAllBlocks(false).sort(byId)
  const unknown = blocks.find((block) => !isRuyiBlockType(block.type))
  ...
  if (visited.size !== blocks.length) {
    return failure(
      'invalid-connection',
      blocks.find((candidate) => !visited.has(candidate.id))?.id ?? null,
    )
  }
