...
function applyValidatedDraft(workspace: Workspace, draft: WorkspaceDraftV1): void {
  workspace.clear()
...
export function loadWorkspaceDraft(workspace: Workspace, draft: WorkspaceDraftV1): void {
  validateDraft(draft)
  const previousDraft = saveWorkspaceDraft(workspace)
  ...
  try {
    ...
    try {
      ...
      throw new AggregateError(
        ...
        'Failed to load workspace draft and failed to restore the previous workspace snapshot',
      )
    }
    throw applyError
  }
