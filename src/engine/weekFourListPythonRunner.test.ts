const SOLVED_WEEK_FOUR_LIST_PYTHON = 'appearances = ["女子", "老妇", "老翁"]\nfor item in appearances:\n    print(item)';
const NESTED_WEEK_FOUR_LIST_PYTHON = SOLVED_WEEK_FOUR_LIST_PYTHON.replace('print(item)', 'print("女子")');
import { readFileSync } from 'node:fs';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
import {
  DEFAULT_WEEK_FOUR_LIST_PYTHON,
  parseWeekFourListPython,
} from './weekFourListPythonGrammar';
import { createWeekFourListPythonRuntime, WeekFourListRuntimeError } from './weekFourListPythonRunner';
...
const readyRuntime = async (options: Parameters<typeof createWeekFourListPythonRuntime>[0] = {}) => {
  const runtime = createWeekFourListPythonRuntime({ ...options, workerFactory: () => new FakeWorker() });
...
describe('W4-M4 isolated branch Python runtime', () => {
  it.each([DEFAULT_WEEK_FOUR_LIST_PYTHON, NESTED_WEEK_FOUR_LIST_PYTHON, SOLVED_WEEK_FOUR_LIST_PYTHON])('accepts the canonical Worker trace for %s', async (code) => {
  ...
  it('prewarms and reports load failure and cold timeout with owned-worker termination', async () => {
    const failed = createWeekFourListPythonRuntime({ workerFactory: () => new FakeWorker() });
    const failure = failed.ready();
    latestWorker().emit({ type: 'load-error', error: 'runtime unavailable' });
    await expect(failure).rejects.toMatchObject({ code: 'load-error' } satisfies Partial<WeekFourListRuntimeError>);
    expect(latestWorker().terminate).toHaveBeenCalledOnce();

    const timed = createWeekFourListPythonRuntime({ coldTimeoutMs: 20, workerFactory: () => new FakeWorker() });
    const timeout = timed.ready();
    const worker = latestWorker();
    const rejected = expect(timeout).rejects.toMatchObject({ code: 'load-timeout' } satisfies Partial<WeekFourListRuntimeError>);
  ...
  it('creates a fresh generation after load failure and ignores every late message from the failed Worker', async () => {
    const runtime = createWeekFourListPythonRuntime({ workerFactory: () => new FakeWorker() });
    ...
    oldWorker.emit({ type: 'result', requestId: 1, trace: runnable(SOLVED_WEEK_FOUR_LIST_PYTHON).trace });
    expect(newWorker.postMessage).not.toHaveBeenCalled();
    newWorker.emit({ type: 'ready' });
    await secondReady;

    const running = runtime.run(SOLVED_WEEK_FOUR_LIST_PYTHON); await Promise.resolve();
    const request = newWorker.postMessage.mock.calls.at(-1)![0];
    let settled = false; running.finally(() => { settled = true; }).catch(() => undefined);
    oldWorker.emit({ type: 'result', requestId: request.requestId, trace: runnable(SOLVED_WEEK_FOUR_LIST_PYTHON).trace });
    await Promise.resolve();
    expect(settled).toBe(false);
    newWorker.emit({ type: 'result', requestId: request.requestId, trace: runnable(SOLVED_WEEK_FOUR_LIST_PYTHON).trace });
    await expect(running).resolves.toMatchObject({ run: { state: 'list-proven', completed: true } });
  });

  it('validates before starting or posting and reserves one cold-start slot', async () => {
    const runtime = createWeekFourListPythonRuntime({ workerFactory: () => new FakeWorker() });
    await expect(runtime.run('print(1)')).rejects.toMatchObject({ code: 'validation' } satisfies Partial<WeekFourListRuntimeError>);
    expect(FakeWorker.instances).toHaveLength(0);

    const first = runtime.run(SOLVED_WEEK_FOUR_LIST_PYTHON);
    const firstWorker = latestWorker();
    const second = runtime.run(DEFAULT_WEEK_FOUR_LIST_PYTHON);
    await expect(second).rejects.toMatchObject({ code: 'busy' } satisfies Partial<WeekFourListRuntimeError>);
    ...
    runtime.cancel();
    await expect(first).rejects.toMatchObject({ code: 'cancelled' } satisfies Partial<WeekFourListRuntimeError>);
  });

  it('times out warm work and cancels both pending and active work', async () => {
    const timed = await readyRuntime({ warmTimeoutMs: 5 });
    const running = timed.run(SOLVED_WEEK_FOUR_LIST_PYTHON);
    const worker = latestWorker();
    const rejected = expect(running).rejects.toMatchObject({ code: 'timeout' } satisfies Partial<WeekFourListRuntimeError>);
    await vi.advanceTimersByTimeAsync(6);
    await rejected;
    expect(worker.terminate).toHaveBeenCalledOnce();

    const pendingRuntime = createWeekFourListPythonRuntime({ workerFactory: () => new FakeWorker() });
    const pending = pendingRuntime.run(DEFAULT_WEEK_FOUR_LIST_PYTHON);
    const pendingWorker = latestWorker();
    pendingRuntime.cancel();
    await expect(pending).rejects.toMatchObject({ code: 'cancelled' } satisfies Partial<WeekFourListRuntimeError>);
    ...
    const active = activeRuntime.run(DEFAULT_WEEK_FOUR_LIST_PYTHON); await Promise.resolve();
    activeRuntime.cancel();
    await expect(active).rejects.toMatchObject({ code: 'cancelled' } satisfies Partial<WeekFourListRuntimeError>);
  });
  ...
  it('preserves same-tick cancellation while a warm run is awaiting its already-ready promise', async () => {
    ...
    const running = runtime.run(SOLVED_WEEK_FOUR_LIST_PYTHON);
    const cancelled = expect(running).rejects.toMatchObject({ code: 'cancelled' } satisfies Partial<WeekFourListRuntimeError>);
  ...
  it('disposes loading and active workers and remains disposed', async () => {
    const loadingRuntime = createWeekFourListPythonRuntime({ workerFactory: () => new FakeWorker() });
    const readiness = loadingRuntime.ready();
    const loadingWorker = latestWorker();
    loadingRuntime.dispose();
    await expect(readiness).rejects.toMatchObject({ code: 'disposed' } satisfies Partial<WeekFourListRuntimeError>);
    expect(loadingWorker.terminate).toHaveBeenCalledOnce();
    await expect(loadingRuntime.ready()).rejects.toMatchObject({ code: 'disposed' } satisfies Partial<WeekFourListRuntimeError>);

    const activeRuntime = await readyRuntime();
    const active = activeRuntime.run(SOLVED_WEEK_FOUR_LIST_PYTHON); await Promise.resolve();
    activeRuntime.dispose();
    await expect(active).rejects.toMatchObject({ code: 'disposed' } satisfies Partial<WeekFourListRuntimeError>);
  });

  it('ignores wrong request ids, late old workers, and edited-input stale results', async () => {
    const runtime = await readyRuntime();
    const firstWorker = latestWorker();
    const first = runtime.run(DEFAULT_WEEK_FOUR_LIST_PYTHON); await Promise.resolve();
    const firstRequest = firstWorker.postMessage.mock.calls.at(-1)![0];
    firstWorker.emit({ type: 'result', requestId: firstRequest.requestId + 1, trace: runnable(DEFAULT_WEEK_FOUR_LIST_PYTHON).trace });
    ...
    const second = runtime.run(SOLVED_WEEK_FOUR_LIST_PYTHON); await Promise.resolve();
    const secondRequest = secondWorker.postMessage.mock.calls.at(-1)![0];
    firstWorker.emit({ type: 'result', requestId: firstRequest.requestId, trace: runnable(DEFAULT_WEEK_FOUR_LIST_PYTHON).trace });
    secondWorker.emit({ type: 'result', requestId: secondRequest.requestId, trace: runnable(DEFAULT_WEEK_FOUR_LIST_PYTHON).trace });
    await expect(second).rejects.toMatchObject({ code: 'worker-contract-mismatch' } satisfies Partial<WeekFourListRuntimeError>);
  });
  ...
  it.each([
    ...
    ['canonical-looking trace mismatch', (worker: FakeWorker, requestId: number) => {
      const trace = runnable(SOLVED_WEEK_FOUR_LIST_PYTHON).trace;
    ...
    await expect(running).rejects.toMatchObject({ code } satisfies Partial<WeekFourListRuntimeError>);
    expect(worker.terminate).toHaveBeenCalledOnce();
  });
  ...
  it('rejects malformed readiness messages instead of trusting unknown data', async () => {
    ...
    await expect(ready).rejects.toMatchObject({ code: 'worker-contract-mismatch' } satisfies Partial<WeekFourListRuntimeError>);
    expect(worker.terminate).toHaveBeenCalledOnce();
  });
});
