const SOLVED_WEEK_FIVE_MONKS_PYTHON = 'monks = ["甲", "乙", "丙"]\nfor monk in monks:\n    release(monk)\n    register(monk)';
const NESTED_WEEK_FIVE_MONKS_PYTHON = SOLVED_WEEK_FIVE_MONKS_PYTHON.replace('release(monk)', 'register(monk)');
import { readFileSync } from 'node:fs';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
import {
  DEFAULT_WEEK_FIVE_MONKS_PYTHON,
  parseWeekFiveMonksPython,
} from './weekFiveMonksPythonGrammar';
import { createWeekFiveMonksPythonRuntime, WeekFiveMonksRuntimeError } from './weekFiveMonksPythonRunner';
...
const readyRuntime = async (options: Parameters<typeof createWeekFiveMonksPythonRuntime>[0] = {}) => {
  const runtime = createWeekFiveMonksPythonRuntime({ ...options, workerFactory: () => new FakeWorker() });
...
describe('W5-M1 isolated branch Python runtime', () => {
  it.each([DEFAULT_WEEK_FIVE_MONKS_PYTHON, NESTED_WEEK_FIVE_MONKS_PYTHON, SOLVED_WEEK_FIVE_MONKS_PYTHON])('accepts the canonical Worker trace for %s', async (code) => {
  ...
  it('prewarms and reports load failure and cold timeout with owned-worker termination', async () => {
    const failed = createWeekFiveMonksPythonRuntime({ workerFactory: () => new FakeWorker() });
    const failure = failed.ready();
    latestWorker().emit({ type: 'load-error', error: 'runtime unavailable' });
    await expect(failure).rejects.toMatchObject({ code: 'load-error' } satisfies Partial<WeekFiveMonksRuntimeError>);
    expect(latestWorker().terminate).toHaveBeenCalledOnce();

    const timed = createWeekFiveMonksPythonRuntime({ coldTimeoutMs: 20, workerFactory: () => new FakeWorker() });
    const timeout = timed.ready();
    const worker = latestWorker();
    const rejected = expect(timeout).rejects.toMatchObject({ code: 'load-timeout' } satisfies Partial<WeekFiveMonksRuntimeError>);
  ...
  it('creates a fresh generation after load failure and ignores every late message from the failed Worker', async () => {
    const runtime = createWeekFiveMonksPythonRuntime({ workerFactory: () => new FakeWorker() });
    ...
    oldWorker.emit({ type: 'result', requestId: 1, trace: runnable(SOLVED_WEEK_FIVE_MONKS_PYTHON).trace });
    expect(newWorker.postMessage).not.toHaveBeenCalled();
    newWorker.emit({ type: 'ready' });
    await secondReady;

    const running = runtime.run(SOLVED_WEEK_FIVE_MONKS_PYTHON); await Promise.resolve();
    const request = newWorker.postMessage.mock.calls.at(-1)![0];
    let settled = false; running.finally(() => { settled = true; }).catch(() => undefined);
    oldWorker.emit({ type: 'result', requestId: request.requestId, trace: runnable(SOLVED_WEEK_FIVE_MONKS_PYTHON).trace });
    await Promise.resolve();
    expect(settled).toBe(false);
    newWorker.emit({ type: 'result', requestId: request.requestId, trace: runnable(SOLVED_WEEK_FIVE_MONKS_PYTHON).trace });
    await expect(running).resolves.toMatchObject({ run: { state: 'rescue-proven', completed: true } });
  });

  it('validates before starting or posting and reserves one cold-start slot', async () => {
    const runtime = createWeekFiveMonksPythonRuntime({ workerFactory: () => new FakeWorker() });
    await expect(runtime.run('print(1)')).rejects.toMatchObject({ code: 'validation' } satisfies Partial<WeekFiveMonksRuntimeError>);
    expect(FakeWorker.instances).toHaveLength(0);

    const first = runtime.run(SOLVED_WEEK_FIVE_MONKS_PYTHON);
    const firstWorker = latestWorker();
    const second = runtime.run(DEFAULT_WEEK_FIVE_MONKS_PYTHON);
    await expect(second).rejects.toMatchObject({ code: 'busy' } satisfies Partial<WeekFiveMonksRuntimeError>);
    ...
    runtime.cancel();
    await expect(first).rejects.toMatchObject({ code: 'cancelled' } satisfies Partial<WeekFiveMonksRuntimeError>);
  });

  it('times out warm work and cancels both pending and active work', async () => {
    const timed = await readyRuntime({ warmTimeoutMs: 5 });
    const running = timed.run(SOLVED_WEEK_FIVE_MONKS_PYTHON);
    const worker = latestWorker();
    const rejected = expect(running).rejects.toMatchObject({ code: 'timeout' } satisfies Partial<WeekFiveMonksRuntimeError>);
    await vi.advanceTimersByTimeAsync(6);
    await rejected;
    expect(worker.terminate).toHaveBeenCalledOnce();

    const pendingRuntime = createWeekFiveMonksPythonRuntime({ workerFactory: () => new FakeWorker() });
    const pending = pendingRuntime.run(DEFAULT_WEEK_FIVE_MONKS_PYTHON);
    const pendingWorker = latestWorker();
    pendingRuntime.cancel();
    await expect(pending).rejects.toMatchObject({ code: 'cancelled' } satisfies Partial<WeekFiveMonksRuntimeError>);
    ...
    const active = activeRuntime.run(DEFAULT_WEEK_FIVE_MONKS_PYTHON); await Promise.resolve();
    activeRuntime.cancel();
    await expect(active).rejects.toMatchObject({ code: 'cancelled' } satisfies Partial<WeekFiveMonksRuntimeError>);
  });
  ...
  it('preserves same-tick cancellation while a warm run is awaiting its already-ready promise', async () => {
    ...
    const running = runtime.run(SOLVED_WEEK_FIVE_MONKS_PYTHON);
    const cancelled = expect(running).rejects.toMatchObject({ code: 'cancelled' } satisfies Partial<WeekFiveMonksRuntimeError>);
  ...
  it('disposes loading and active workers and remains disposed', async () => {
    const loadingRuntime = createWeekFiveMonksPythonRuntime({ workerFactory: () => new FakeWorker() });
    const readiness = loadingRuntime.ready();
    const loadingWorker = latestWorker();
    loadingRuntime.dispose();
    await expect(readiness).rejects.toMatchObject({ code: 'disposed' } satisfies Partial<WeekFiveMonksRuntimeError>);
    expect(loadingWorker.terminate).toHaveBeenCalledOnce();
    await expect(loadingRuntime.ready()).rejects.toMatchObject({ code: 'disposed' } satisfies Partial<WeekFiveMonksRuntimeError>);

    const activeRuntime = await readyRuntime();
    const active = activeRuntime.run(SOLVED_WEEK_FIVE_MONKS_PYTHON); await Promise.resolve();
    activeRuntime.dispose();
    await expect(active).rejects.toMatchObject({ code: 'disposed' } satisfies Partial<WeekFiveMonksRuntimeError>);
  });

  it('ignores wrong request ids, late old workers, and edited-input stale results', async () => {
    const runtime = await readyRuntime();
    const firstWorker = latestWorker();
    const first = runtime.run(DEFAULT_WEEK_FIVE_MONKS_PYTHON); await Promise.resolve();
    const firstRequest = firstWorker.postMessage.mock.calls.at(-1)![0];
    firstWorker.emit({ type: 'result', requestId: firstRequest.requestId + 1, trace: runnable(DEFAULT_WEEK_FIVE_MONKS_PYTHON).trace });
    ...
    const second = runtime.run(SOLVED_WEEK_FIVE_MONKS_PYTHON); await Promise.resolve();
    const secondRequest = secondWorker.postMessage.mock.calls.at(-1)![0];
    firstWorker.emit({ type: 'result', requestId: firstRequest.requestId, trace: runnable(DEFAULT_WEEK_FIVE_MONKS_PYTHON).trace });
    secondWorker.emit({ type: 'result', requestId: secondRequest.requestId, trace: runnable(DEFAULT_WEEK_FIVE_MONKS_PYTHON).trace });
    await expect(second).rejects.toMatchObject({ code: 'worker-contract-mismatch' } satisfies Partial<WeekFiveMonksRuntimeError>);
  });
  ...
  it.each([
    ...
    ['canonical-looking trace mismatch', (worker: FakeWorker, requestId: number) => {
      const trace = runnable(SOLVED_WEEK_FIVE_MONKS_PYTHON).trace;
    ...
    await expect(running).rejects.toMatchObject({ code } satisfies Partial<WeekFiveMonksRuntimeError>);
    expect(worker.terminate).toHaveBeenCalledOnce();
  });
  ...
  it('rejects malformed readiness messages instead of trusting unknown data', async () => {
    ...
    await expect(ready).rejects.toMatchObject({ code: 'worker-contract-mismatch' } satisfies Partial<WeekFiveMonksRuntimeError>);
    expect(worker.terminate).toHaveBeenCalledOnce();
  });
});
