...
import { DEFAULT_WEEK_FIVE_DECOMPOSITION_PYTHON, SOLVED_WEEK_FIVE_DECOMPOSITION_PYTHON } from '../src/engine/weekFiveDecompositionPythonGrammar';
import { formalW5M3Prerequisite } from './support/w5m4Prerequisite';
import { collectRuntimeClosure, WEEK_FIVE_DECOMPOSITION_MAX_LAZY_BYTES } from '../scripts/check-bundle-budget.mjs';
...
const DEFAULT = DEFAULT_WEEK_FIVE_DECOMPOSITION_PYTHON;
const SOLVED = SOLVED_WEEK_FIVE_DECOMPOSITION_PYTHON;
const OWNERSHIP_FIXED = DEFAULT.replace("    record_trial('隔板猜物')\n\n", '\n');
const saved = (page: Page) => page.evaluate((key) => JSON.parse(localStorage.getItem(key)!), KEY);
const session = async (page: Page) => (await saved(page)).sessions['w5-m4'];
const runButton = (page: Page) => page.getByRole('button', { name: '运行问题分解代码', exact: true });

async function seed(page: Page, raw = formalW5M3Prerequisite()) {
...
async function open(page: Page) {
  await page.goto('./#/mission/w5-m4');
  await expect(page.getByLabel('W5-M4 Python 问题分解代码')).toBeVisible();
  await expect(runButton(page).or(page.getByRole('button', { name: '真实回放分解作品', exact: true }))).toBeEnabled({ timeout: 30_000 });
  await expect(page.getByRole('status', { name: 'Python 运行环境已准备' })).toBeVisible({ timeout: 30_000 });
}
async function setCode(page: Page, code: string) {
  await page.getByLabel('W5-M4 Python 问题分解代码').fill(code);
...
test('@w5-m4-full real decomposition reports ownership first, exposes the missing call, seals proof, restores and replays readonly', async ({ page }, info) => {
  test.setTimeout(150_000);
  const errors: string[] = [];
  page.on('pageerror', (error) => errors.push(error.message));
  await seed(page); await open(page);
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await expect(page.locator('img[alt*="空白记录册"]')).toHaveJSProperty('complete', true);
  await page.screenshot({ path: info.outputPath(`w5m4-default-${info.project.name}.png`), fullPage: true });

  await run(page);
  expect((await session(page)).lastRun).toMatchObject({ state: 'record-ownership-conflict', penalty: { livesLost: 0, resourcesLost: 0, starsLost: 0 } });
  expect((await session(page)).failureSnapshot.sourceSpans[0].line).toBe(3);
  await page.getByRole('button', { name: /火眼金睛/ }).click();
  await expect(page.getByRole('status', { name: '已保存的问题分解实际执行' })).toContainText('坐禅记录组实际有');

  await setCode(page, OWNERSHIP_FIXED); await run(page);
  expect((await session(page)).lastRun.state).toBe('coordinator-call-conflict');
  await setCode(page, SOLVED.replace('    record_guess()', '    record_guess()\n    record_guess()')); await run(page);
  expect((await session(page)).lastRun.state).toBe('coordinator-call-conflict');
  ...
  await expect(page.locator('.week-five-temple-epilogue').getByText(/三位国师在后续比试中先后败亡/)).toBeVisible();
  await page.getByRole('button', { name: '真实回放分解作品' }).click();
  await expect(page.getByRole('dialog', { name: '闯关成功' })).toBeVisible({ timeout: 30_000 });
  expect(await saved(page)).toEqual(replayBefore);

  await openParent(page);
  const summary = page.getByRole('region', { name: '第五周问题分解学习摘要' });
  await expect(summary).toContainText('问题分解正式证明及故事记录作品已保存');
  ...
  await expect.poll(async () => (await saved(page)).missions['w5-m4']).toBeUndefined();
  await expect(page.getByLabel('设置 4 位家长 PIN', { exact: true })).toBeVisible({ timeout: 30_000 });
  await unlockParent(page);
  await page.getByLabel('选择进度文件').setInputFiles({ name: 'w5m4.json', mimeType: 'application/json', buffer: bytes });
  await expect.poll(async () => (await saved(page)).missionCompletionEvidence['w5-m4']?.kind).toBe('formal-v3');
  await page.goto('./#/mission/w5-m5');
  await expect(page.getByRole('heading', { name: '车迟国总试炼', exact: true })).toBeVisible();
  expect(errors).toEqual([]);
});

test('@w5-m4-keyboard CodeMirror stays keyboard reachable and helper controls do not mutate the work', async ({ page }) => {
  await seed(page); await open(page);
  const editor = page.getByLabel('W5-M4 Python 问题分解代码');
  ...
  await editor.press('Tab'); await expect(page.getByRole('button', { name: '定位记录归属' })).toBeFocused();
  const before = await session(page);
  await page.getByRole('button', { name: '定位记录归属' }).click(); await expect(editor).toBeFocused();
  await page.getByRole('button', { name: '定位总记录' }).click(); await expect(editor).toBeFocused();
  expect((await session(page)).pythonCode).toBe(before.pythonCode);
  await runButton(page).focus(); await runButton(page).press('Enter');
  await expect(page.getByRole('dialog', { name: '闯关成功' })).toBeVisible({ timeout: 30_000 });
});

for (const stage of ['run', 'validation', 'observation', 'work', 'completion'] as const) test(`@w5-m4-storage ${stage} failure retries the exact transaction`, async ({ page }) => {
  await seed(page); await open(page);
  ...
  await page.evaluate(({ mode, stage }) => localStorage.setItem(mode, `fail-w5-m4-${stage === 'validation' ? 'run' : stage}`), { mode: MODE, stage });
...
test('@w5-m4-security actual built Worker rejects unsupported syntax and accepts only the real solved trace', async ({ page }) => {
  await seed(page); await open(page); const before = await saved(page);
  const workers = readdirSync('dist-e2e/assets').filter((file) => /^weekFiveDecompositionPython\.worker-.*\.js$/.test(file));
  ...
  expect(result[3].trace.filter((item: any) => item.kind === 'action')).toHaveLength(5);
  expect(await saved(page)).toEqual(before);
});

test('@w5-m4-assets missing courtyard blocks proof and retry completes without rerunning Python', async ({ page }) => {
  ...
  expect((await session(page)).lastRun.completed).toBe(true); expect((await saved(page)).missions['w5-m4']).toBeUndefined(); expect(requests).toBeGreaterThan(0);
  await page.unroute(pattern); await page.getByRole('button', { name: '重试加载故事记录庭院' }).click();
  await expect(page.getByRole('dialog', { name: '闯关成功' })).toBeVisible({ timeout: 30_000 });
  expect((await session(page)).totalRuns).toBe(1);
});

test('@w5-m4-corrupt restores the formal snapshot, preserves damaged source, and readonly replay remains exact', async ({ page }) => {
  await seed(page); await open(page); await setCode(page, SOLVED); await runButton(page).click();
  await expect(page.getByRole('dialog', { name: '闯关成功' })).toBeVisible({ timeout: 30_000 });
  const completed = await saved(page);
  await page.evaluate((mode) => localStorage.setItem(mode, 'corrupt-w5-m4-current'), MODE); await page.reload();
  await expect(page.getByText('学习进度已经安全恢复')).toBeVisible();
  const recovered = await saved(page);
  expect(recovered.recovery.source).toBe('snapshot'); expect(recovered.works['w5-m4-problem-decomposition-record']).toEqual(completed.works['w5-m4-problem-decomposition-record']);
  await open(page); const before = await saved(page); await page.getByRole('button', { name: '真实回放分解作品' }).click();
  await expect(page.getByRole('dialog', { name: '闯关成功' })).toBeVisible({ timeout: 30_000 }); expect(await saved(page)).toEqual(before);
});

test('@w5-m4-external a stale tab cannot overwrite and explicitly reloads the newer draft', async ({ page, context }) => {
  ...
  const second = await context.newPage(); await open(second); await setCode(second, OWNERSHIP_FIXED);
  ...
  await expect.poll(async () => (await session(page)).pythonCode).toBe(OWNERSHIP_FIXED);
});

test('@w5-m4-runtime Pyodide load failure is infrastructure and a later real Worker run recovers', async ({ page }) => {
  ...
  await page.unroute(pattern); await run(page); expect((await session(page)).lastRun.state).toBe('record-ownership-conflict');
});

test('@w5-m4-runtime timeout counts one attempted run and explicit cancel discards a delayed result', async ({ page }) => {
  await seed(page); let delivered = 0; const pattern = /weekFiveDecompositionPython\.worker-[^/]+\.js/;
...
test('@w5-m4-legacy revision 15 completion stays legacy until a real solved run upgrades the proof', async ({ page }) => {
  const old = JSON.parse(formalW5M3Prerequisite()); old.schemaRevision = 15; old.missions['w5-m4'] = { status: 'completed', stars: 2, hintsUsed: 1, attempts: 2, completedAt: '2026-09-11T00:00:20.000Z' };
  await seed(page, JSON.stringify(old)); await open(page);
  expect((await saved(page)).missionCompletionEvidence['w5-m4'].kind).toBe('legacy-replay-only');
  await setCode(page, SOLVED); await runButton(page).click();
  await expect(page.getByRole('dialog', { name: '闯关成功' })).toBeVisible({ timeout: 30_000 });
  const current = await saved(page); expect(current.missions['w5-m4']).toEqual(old.missions['w5-m4']); expect(current.missionCompletionEvidence['w5-m4'].kind).toBe('formal-v3');
});

test('@w5-m4-cold lazy closure, media, built Worker, local Pyodide, first and warm real results stay within inherited budgets', async ({ page, context }, info) => {
  ...
  const closureFiles = [...collectRuntimeClosure(manifest, 'src/components/WeekFiveDecompositionExperience.tsx')].map((key) => manifest[key]?.file).filter((file): file is string => typeof file === 'string' && file.endsWith('.js'));
  const closureBytes = closureFiles.reduce((sum, file) => sum + statSync(new URL(`../dist-e2e/${file}`, import.meta.url)).size, 0);
  const workers = readdirSync(new URL('../dist-e2e/assets/', import.meta.url)).filter((file) => /^weekFiveDecompositionPython\.worker-.*\.js$/.test(file)); expect(workers).toHaveLength(1);
  ...
  expect(resources.some((url) => url.includes('weekFiveDecompositionPython.worker-'))).toBe(true);
  ...
  expect(closureBytes + workerBytes).toBeLessThanOrEqual(WEEK_FIVE_DECOMPOSITION_MAX_LAZY_BYTES);
  ...
  const metricsPath = info.outputPath(`w5m4-cold-${info.project.name}.json`);
  ...
  await info.attach(`w5m4-cold-${info.project.name}.json`, { path: metricsPath, contentType: 'application/json' });
});
