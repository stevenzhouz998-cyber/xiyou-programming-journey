...
function hasNoW4VariablePublication(progress: ProgressV3) {
  return progress.missions['w4-m2'] === undefined
    && progress.missionCompletionEvidence['w4-m2'] === undefined
    && progress.works['w4-m2-variable-evidence-record'] === undefined;
}

function exactW4VariableDraftDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w4-m2']; const candidate = next.sessions['w4-m2'];
  if (!candidate || !hasNoW4VariablePublication(next) || candidate.lastRun !== null) return false;
  if (!prior) return exactAfterAllowedDelta(previous, next, (expected) => { expected.sessions['w4-m2'] = structuredClone(candidate); });
  return exactAfterAllowedDelta(previous, next, (expected) => {
    const session = expected.sessions['w4-m2']!;
    session.pythonCode = candidate.pythonCode;
    session.pythonSourceSpan = structuredClone(candidate.pythonSourceSpan);
...
function exactW4VariableRunDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w4-m2']; const candidate = next.sessions['w4-m2'];
  if (!prior || !candidate || !hasNoW4VariablePublication(next)) return false;
  if (candidate.validationFailures === prior.validationFailures + 1) {
    return exactAfterAllowedDelta(previous, next, (expected) => {
      const session = expected.sessions['w4-m2']!;
      session.validationFailures = candidate.validationFailures;
  ...
  return exactAfterAllowedDelta(previous, next, (expected) => {
    ...
    session.failureSnapshot = structuredClone(candidate.failureSnapshot);
    session.totalRuns = candidate.totalRuns;
    session.overwriteFailures = candidate.overwriteFailures;
    session.conceptFailures = structuredClone(candidate.conceptFailures);
    session.lastRunAt = candidate.lastRunAt;
    session.savedAt = candidate.savedAt;
  });
...
function exactW4VariableObservationDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w4-m2']; const candidate = next.sessions['w4-m2'];
  const observation = candidate?.conditionObservationUses.at(-1);
  if (!prior || !candidate || !observation || !prior.failureSnapshot || !hasNoW4VariablePublication(next)
    ...
    || observation.pythonCode !== prior.pythonCode
    || observation.usedAt !== candidate.savedAt) return false;
  return exactAfterAllowedDelta(previous, next, (expected) => {
    const session = expected.sessions['w4-m2']!;
...
function exactW4VariableCompletionDelta(previous: ProgressV3, next: ProgressV3) {
  const completion = next.missions['w4-m2']; const evidence = next.missionCompletionEvidence['w4-m2']; const work = next.works['w4-m2-variable-evidence-record'];
  const oldEvidence = previous.missionCompletionEvidence['w4-m2'];
  const initial = previous.missions['w4-m2'] === undefined && oldEvidence === undefined && previous.works['w4-m2-variable-evidence-record'] === undefined;
  const upgrade = previous.missions['w4-m2'] !== undefined && oldEvidence?.kind === 'legacy-replay-only' && previous.works['w4-m2-variable-evidence-record'] === undefined;
  if (!completion || evidence?.kind !== 'formal-v3' || !work || (!initial && !upgrade)) return false;
  return exactAfterAllowedDelta(previous, next, (expected) => {
    if (initial) expected.missions['w4-m2'] = structuredClone(completion);
...
function hasNoW4BossPublication(progress: ProgressV3) {
  return progress.missions['w4-m5'] === undefined
    && progress.missionCompletionEvidence['w4-m5'] === undefined
    && progress.works['w4-m5-verification-report'] === undefined;
}

function exactW4BossDraftDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w4-m5']; const candidate = next.sessions['w4-m5'];
  if (!candidate || !hasNoW4BossPublication(next) || candidate.lastRun !== null) return false;
  if (!prior) return exactAfterAllowedDelta(previous, next, (expected) => { expected.sessions['w4-m5'] = structuredClone(candidate); });
  return exactAfterAllowedDelta(previous, next, (expected) => {
    const session = expected.sessions['w4-m5']!;
    session.pythonCode = candidate.pythonCode;
    session.lastCanonicalTrace = [];
    session.lastWorkerTrace = [];
    session.lastRun = null;
...
function exactW4BossRunDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w4-m5']; const candidate = next.sessions['w4-m5'];
  if (!prior || !candidate || !hasNoW4BossPublication(next)) return false;
  if (candidate.validationFailures === prior.validationFailures + 1) {
    return exactAfterAllowedDelta(previous, next, (expected) => {
      const session = expected.sessions['w4-m5']!;
      session.validationFailures = candidate.validationFailures;
  ...
  return exactAfterAllowedDelta(previous, next, (expected) => {
    ...
    session.conditionObservationUses = [];
    session.totalRuns = candidate.totalRuns;
    session.identityFailures = candidate.identityFailures;
    session.branchFailures = candidate.branchFailures;
    session.firstBlockingConcept = candidate.firstBlockingConcept;
    session.lastRunAt = candidate.lastRunAt;
    session.savedAt = candidate.savedAt;
  });
...
function exactW4BossObservationDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w4-m5']; const candidate = next.sessions['w4-m5'];
  const observation = candidate?.conditionObservationUses.at(-1);
  if (!prior || !candidate || !observation || !prior.failureSnapshot || !hasNoW4BossPublication(next)
    || candidate.conditionObservationUses.length !== prior.conditionObservationUses.length + 1
    || canonicalJson(candidate.conditionObservationUses.slice(0, -1)) !== canonicalJson(prior.conditionObservationUses)
    || prior.conditionObservationUses.some((use) => use.snapshotId === observation.snapshotId)
    || observation.snapshotId !== prior.failureSnapshot.snapshotId
...
function exactW4BossCompletionDelta(previous: ProgressV3, next: ProgressV3) {
  const completion = next.missions['w4-m5'];
  const evidence = next.missionCompletionEvidence['w4-m5'];
  const work = next.works['w4-m5-verification-report'];
  const oldEvidence = previous.missionCompletionEvidence['w4-m5'];
  const initial = previous.missions['w4-m5'] === undefined && oldEvidence === undefined
    && previous.works['w4-m5-verification-report'] === undefined;
  const upgrade = previous.missions['w4-m5'] !== undefined && oldEvidence?.kind === 'legacy-replay-only'
    && previous.works['w4-m5-verification-report'] === undefined;
  if (!completion || evidence?.kind !== 'formal-v3' || !work || (!initial && !upgrade)) return false;
  return exactAfterAllowedDelta(previous, next, (expected) => {
    if (initial) expected.missions['w4-m5'] = structuredClone(completion);
    expected.missionCompletionEvidence['w4-m5'] = structuredClone(evidence);
    expected.works['w4-m5-verification-report'] = structuredClone(work);
  });
}
...
function hasNoW5FunctionPublication(progress: ProgressV3) {
  return progress.missions['w5-m2'] === undefined
    && progress.missionCompletionEvidence['w5-m2'] === undefined
    && progress.works['w5-m2-sanqing-function-record'] === undefined;
}

function exactW5FunctionDraftDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w5-m2']; const candidate = next.sessions['w5-m2'];
  if (!candidate || !hasNoW5FunctionPublication(next) || candidate.lastRun !== null) return false;
  if (!prior) return exactAfterAllowedDelta(previous, next, (expected) => { expected.sessions['w5-m2'] = structuredClone(candidate); });
  return exactAfterAllowedDelta(previous, next, (expected) => {
    const session = expected.sessions['w5-m2']!;
    session.pythonCode = candidate.pythonCode;
    session.lastCanonicalTrace = [];
    session.lastWorkerTrace = [];
    session.lastRun = null;
...
function exactW5FunctionRunDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w5-m2']; const candidate = next.sessions['w5-m2'];
  if (!prior || !candidate || !hasNoW5FunctionPublication(next)) return false;
  if (candidate.validationFailures === prior.validationFailures + 1) {
    return exactAfterAllowedDelta(previous, next, (expected) => {
      const session = expected.sessions['w5-m2']!;
      session.validationFailures = candidate.validationFailures;
  ...
  return exactAfterAllowedDelta(previous, next, (expected) => {
    ...
    session.conditionObservationUses = [];
    session.totalRuns = candidate.totalRuns;
    session.callFailures = candidate.callFailures;
    session.bodyFailures = candidate.bodyFailures;
    session.firstBlockingConcept = candidate.firstBlockingConcept;
    session.lastRunAt = candidate.lastRunAt;
    session.savedAt = candidate.savedAt;
  });
...
function exactW5FunctionObservationDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w5-m2']; const candidate = next.sessions['w5-m2'];
  const observation = candidate?.conditionObservationUses.at(-1);
  if (!prior || !candidate || !observation || !prior.failureSnapshot || !hasNoW5FunctionPublication(next)
    || candidate.conditionObservationUses.length !== prior.conditionObservationUses.length + 1
    || canonicalJson(candidate.conditionObservationUses.slice(0, -1)) !== canonicalJson(prior.conditionObservationUses)
    || prior.conditionObservationUses.some((use) => use.snapshotId === observation.snapshotId)
    || observation.snapshotId !== prior.failureSnapshot.snapshotId
...
function exactW5FunctionCompletionDelta(previous: ProgressV3, next: ProgressV3) {
  const completion = next.missions['w5-m2'];
  const evidence = next.missionCompletionEvidence['w5-m2'];
  const work = next.works['w5-m2-sanqing-function-record'];
  const oldEvidence = previous.missionCompletionEvidence['w5-m2'];
  const initial = previous.missions['w5-m2'] === undefined && oldEvidence === undefined
    && previous.works['w5-m2-sanqing-function-record'] === undefined;
  const upgrade = previous.missions['w5-m2'] !== undefined && oldEvidence?.kind === 'legacy-replay-only'
    && previous.works['w5-m2-sanqing-function-record'] === undefined;
  if (!completion || evidence?.kind !== 'formal-v3' || !work || (!initial && !upgrade)) return false;
  return exactAfterAllowedDelta(previous, next, (expected) => {
    if (initial) expected.missions['w5-m2'] = structuredClone(completion);
    expected.missionCompletionEvidence['w5-m2'] = structuredClone(evidence);
    expected.works['w5-m2-sanqing-function-record'] = structuredClone(work);
  });
}

function hasNoW5WeatherPublication(progress: ProgressV3) {
  return progress.missions['w5-m3'] === undefined
    && progress.missionCompletionEvidence['w5-m3'] === undefined
    && progress.works['w5-m3-weather-parameter-record'] === undefined;
}

function exactW5WeatherDraftDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w5-m3']; const candidate = next.sessions['w5-m3'];
  if (!candidate || !hasNoW5WeatherPublication(next) || candidate.lastRun !== null) return false;
  if (!prior) return exactAfterAllowedDelta(previous, next, (expected) => { expected.sessions['w5-m3'] = structuredClone(candidate); });
  return exactAfterAllowedDelta(previous, next, (expected) => {
    const session = expected.sessions['w5-m3']!;
    session.pythonCode = candidate.pythonCode;
    session.lastCanonicalTrace = [];
    session.lastWorkerTrace = [];
    session.lastRun = null;
...
function exactW5WeatherRunDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w5-m3']; const candidate = next.sessions['w5-m3'];
  if (!prior || !candidate || !hasNoW5WeatherPublication(next)) return false;
  if (candidate.validationFailures === prior.validationFailures + 1) {
    return exactAfterAllowedDelta(previous, next, (expected) => {
      const session = expected.sessions['w5-m3']!;
      session.validationFailures = candidate.validationFailures;
  ...
  return exactAfterAllowedDelta(previous, next, (expected) => {
    ...
    session.totalRuns = candidate.totalRuns;
    session.callFailures = candidate.callFailures;
    session.parameterFailures = candidate.parameterFailures;
    session.firstBlockingConcept = candidate.firstBlockingConcept;
    session.lastRunAt = candidate.lastRunAt;
    session.savedAt = candidate.savedAt;
  });
...
function exactW5WeatherObservationDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w5-m3']; const candidate = next.sessions['w5-m3'];
  const observation = candidate?.conditionObservationUses.at(-1);
  if (!prior || !candidate || !observation || !prior.failureSnapshot || !hasNoW5WeatherPublication(next)
    || candidate.conditionObservationUses.length !== prior.conditionObservationUses.length + 1
    || canonicalJson(candidate.conditionObservationUses.slice(0, -1)) !== canonicalJson(prior.conditionObservationUses)
    || prior.conditionObservationUses.some((use) => use.snapshotId === observation.snapshotId)
    || observation.snapshotId !== prior.failureSnapshot.snapshotId
...
function exactW5WeatherCompletionDelta(previous: ProgressV3, next: ProgressV3) {
  const completion = next.missions['w5-m3'];
  const evidence = next.missionCompletionEvidence['w5-m3'];
  const work = next.works['w5-m3-weather-parameter-record'];
  const oldEvidence = previous.missionCompletionEvidence['w5-m3'];
  const initial = previous.missions['w5-m3'] === undefined && oldEvidence === undefined
    && previous.works['w5-m3-weather-parameter-record'] === undefined;
  const upgrade = previous.missions['w5-m3'] !== undefined && oldEvidence?.kind === 'legacy-replay-only'
    && previous.works['w5-m3-weather-parameter-record'] === undefined;
  if (!completion || evidence?.kind !== 'formal-v3' || !work || (!initial && !upgrade)) return false;
  return exactAfterAllowedDelta(previous, next, (expected) => {
    if (initial) expected.missions['w5-m3'] = structuredClone(completion);
    expected.missionCompletionEvidence['w5-m3'] = structuredClone(evidence);
    expected.works['w5-m3-weather-parameter-record'] = structuredClone(work);
  });
}


function hasNoW5DecompositionPublication(progress: ProgressV3) {
  return progress.missions['w5-m4'] === undefined
    && progress.missionCompletionEvidence['w5-m4'] === undefined
    && progress.works['w5-m4-problem-decomposition-record'] === undefined;
}

function exactW5DecompositionDraftDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w5-m4']; const candidate = next.sessions['w5-m4'];
  if (!candidate || !hasNoW5DecompositionPublication(next) || candidate.lastRun !== null) return false;
  if (!prior) return exactAfterAllowedDelta(previous, next, (expected) => { expected.sessions['w5-m4'] = structuredClone(candidate); });
  return exactAfterAllowedDelta(previous, next, (expected) => {
    const session = expected.sessions['w5-m4']!;
    session.pythonCode = candidate.pythonCode;
    session.lastCanonicalTrace = [];
    session.lastWorkerTrace = [];
    session.lastRun = null;
...
function exactW5DecompositionRunDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w5-m4']; const candidate = next.sessions['w5-m4'];
  if (!prior || !candidate || !hasNoW5DecompositionPublication(next)) return false;
  if (candidate.validationFailures === prior.validationFailures + 1) {
    return exactAfterAllowedDelta(previous, next, (expected) => {
      const session = expected.sessions['w5-m4']!;
      session.validationFailures = candidate.validationFailures;
  ...
  return exactAfterAllowedDelta(previous, next, (expected) => {
    ...
    session.conditionObservationUses = [];
    session.totalRuns = candidate.totalRuns;
    session.coordinatorFailures = candidate.coordinatorFailures;
    session.ownershipFailures = candidate.ownershipFailures;
    session.firstBlockingConcept = candidate.firstBlockingConcept;
    session.lastRunAt = candidate.lastRunAt;
    session.savedAt = candidate.savedAt;
  });
...
function exactW5DecompositionObservationDelta(previous: ProgressV3, next: ProgressV3) {
  const prior = previous.sessions['w5-m4']; const candidate = next.sessions['w5-m4'];
  const observation = candidate?.conditionObservationUses.at(-1);
  if (!prior || !candidate || !observation || !prior.failureSnapshot || !hasNoW5DecompositionPublication(next)
    || candidate.conditionObservationUses.length !== prior.conditionObservationUses.length + 1
    || canonicalJson(candidate.conditionObservationUses.slice(0, -1)) !== canonicalJson(prior.conditionObservationUses)
    || prior.conditionObservationUses.some((use) => use.snapshotId === observation.snapshotId)
    || observation.snapshotId !== prior.failureSnapshot.snapshotId
...
function exactW5DecompositionCompletionDelta(previous: ProgressV3, next: ProgressV3) {
  const completion = next.missions['w5-m4'];
  const evidence = next.missionCompletionEvidence['w5-m4'];
  const work = next.works['w5-m4-problem-decomposition-record'];
  const oldEvidence = previous.missionCompletionEvidence['w5-m4'];
  const initial = previous.missions['w5-m4'] === undefined && oldEvidence === undefined
    && previous.works['w5-m4-problem-decomposition-record'] === undefined;
  const upgrade = previous.missions['w5-m4'] !== undefined && oldEvidence?.kind === 'legacy-replay-only'
    && previous.works['w5-m4-problem-decomposition-record'] === undefined;
  if (!completion || evidence?.kind !== 'formal-v3' || !work || (!initial && !upgrade)) return false;
  return exactAfterAllowedDelta(previous, next, (expected) => {
    if (initial) expected.missions['w5-m4'] = structuredClone(completion);
    expected.missionCompletionEvidence['w5-m4'] = structuredClone(evidence);
    expected.works['w5-m4-problem-decomposition-record'] = structuredClone(work);
  });
}
...
function hasNoW5StoryPublication(progress: ProgressV3) {
  return progress.missions['w5-m5'] === undefined
    && progress.missionCompletionEvidence['w5-m5'] === undefined
    && progress.works['w5-m5-story-orchestration-record'] === undefined;
}

function exactW5StorySessionDelta(previous: ProgressV3, next: ProgressV3, stage: 'draft' | 'run' | 'observation') {
  const prior = previous.sessions['w5-m5']; const candidate = next.sessions['w5-m5'];
  if (!candidate || !hasNoW5StoryPublication(next)) return false;
  ...
  return exactAfterAllowedDelta(previous, next, (expected) => { expected.sessions['w5-m5'] = structuredClone(candidate); });
}

function exactW5StoryCompletionDelta(previous: ProgressV3, next: ProgressV3) {
  const completion = next.missions['w5-m5'];
  const evidence = next.missionCompletionEvidence['w5-m5'];
  const work = next.works['w5-m5-story-orchestration-record'];
  const oldEvidence = previous.missionCompletionEvidence['w5-m5'];
  const initial = previous.missions['w5-m5'] === undefined && oldEvidence === undefined && previous.works['w5-m5-story-orchestration-record'] === undefined;
  const upgrade = previous.missions['w5-m5'] !== undefined && oldEvidence?.kind === 'legacy-replay-only' && previous.works['w5-m5-story-orchestration-record'] === undefined;
  if (!completion || evidence?.kind !== 'formal-v3' || !work || (!initial && !upgrade)) return false;
  return exactAfterAllowedDelta(previous, next, (expected) => {
    if (initial) expected.missions['w5-m5'] = structuredClone(completion);
    expected.missionCompletionEvidence['w5-m5'] = structuredClone(evidence);
    expected.works['w5-m5-story-orchestration-record'] = structuredClone(work);
  });
}
...
function hasNoW6ArchivePublication(progress:ProgressV3){return progress.missions['w6-m5']===undefined&&progress.missionCompletionEvidence['w6-m5']===undefined&&progress.works['w6-m5-journey-archive']===undefined}
function exactW6ArchiveSessionDelta(previous:ProgressV3,next:ProgressV3){const candidate=next.sessions['w6-m5'];return !!candidate&&hasNoW6ArchivePublication(next)&&exactAfterAllowedDelta(previous,next,expected=>{expected.sessions['w6-m5']=structuredClone(candidate)})}
function exactW6ArchiveCompletionDelta(previous:ProgressV3,next:ProgressV3){const completion=next.missions['w6-m5'],evidence=next.missionCompletionEvidence['w6-m5'],work=next.works['w6-m5-journey-archive'],oldEvidence=previous.missionCompletionEvidence['w6-m5'];const initial=hasNoW6ArchivePublication(previous),upgrade=previous.missions['w6-m5']!==undefined&&oldEvidence?.kind==='legacy-replay-only'&&previous.works['w6-m5-journey-archive']===undefined;if(!completion||evidence?.kind!=='formal-v3'||!work||(!initial&&!upgrade))return false;return exactAfterAllowedDelta(previous,next,expected=>{if(initial)expected.missions['w6-m5']=structuredClone(completion);expected.missionCompletionEvidence['w6-m5']=structuredClone(evidence);expected.works['w6-m5-journey-archive']=structuredClone(work)})}
...
export const storageFaultAdapter: StorageFaultAdapter = {
  beforeProgressWrite: ({ storage, progress }) => {
    ...
    if (mode === 'fail-w4-m2-draft' && exactW4VariableDraftDelta(previous, progress)) return FAILURE;
    if (mode === 'fail-w4-m2-run' && exactW4VariableRunDelta(previous, progress)) return FAILURE;
    ...
    if (mode === 'fail-w5-m2-draft' && exactW5FunctionDraftDelta(previous, progress)) return FAILURE;
    ...
    if (mode === 'fail-w6-m5-session' && exactW6ArchiveSessionDelta(previous, progress)) return FAILURE;
    ...
    if (mode === 'fail-w5-m2-run' && exactW5FunctionRunDelta(previous, progress)) return FAILURE;
    ...
    if (mode === 'fail-w5-m2-observation' && exactW5FunctionObservationDelta(previous, progress)) return FAILURE;
    ...
    if (mode === 'fail-w5-m2-work' && exactW5FunctionCompletionDelta(previous, progress)) return FAILURE;
    ...
    if (mode === 'fail-w5-m2-completion' && exactW5FunctionCompletionDelta(previous, progress)) return FAILURE;
    ...
    if (mode === 'fail-w6-m5-completion' && exactW6ArchiveCompletionDelta(previous, progress)) return FAILURE;
