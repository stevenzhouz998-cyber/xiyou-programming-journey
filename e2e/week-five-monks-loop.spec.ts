...
test('@w5-m1-full actual loop, independent bugs, refresh, readonly replay, parent export-clear-import and unlock',async({page},info)=>{
  ...
  const completed=await saved(page);expect(completed.schemaRevision).toBe(13);expect(completed.missionCompletionEvidence['w5-m1'].kind).toBe('formal-v3');expect(completed.works['w5-m1-monks-rescue-record'].run.completed).toBe(true);
