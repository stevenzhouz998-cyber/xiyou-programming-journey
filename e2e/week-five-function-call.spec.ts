...
import { collectRuntimeClosure, WEEK_FIVE_FUNCTION_MAX_LAZY_BYTES } from '../scripts/check-bundle-budget.mjs';
import { PYTHON_RUNTIME_TRANSFER_MAX_BYTES } from '../scripts/budget-limits.mjs';
import { formalW5M1Prerequisite } from './support/w5m2Prerequisite';
...
const DEFAULT = 'def record_sanqing():\n    record_arrival()\n    record_names()';
const SOLVED = `${DEFAULT}\n\nrecord_sanqing()`;
const saved = (page: Page) => page.evaluate((key) => JSON.parse(localStorage.getItem(key)!), KEY);
const session = async (page: Page) => (await saved(page)).sessions['w5-m2'];
const runButton = (page: Page) => page.getByRole('button', { name: '运行三清观函数', exact: true });

async function seed(page: Page, raw = formalW5M1Prerequisite()) {
...
async function open(page: Page) {
  await page.goto('./#/mission/w5-m2');
  await expect(page.getByLabel('W5-M2 Python 函数代码')).toBeVisible();
  await expect(runButton(page).or(page.getByRole('button', { name: '真实回放函数作品', exact: true }))).toBeEnabled({ timeout: 30_000 });
  await expect(page.getByRole('status', { name: 'Python 运行环境已准备' })).toBeVisible({ timeout: 30_000 });
}
async function setCode(page: Page, code: string) {
  await page.getByLabel('W5-M2 Python 函数代码').fill(code);
...
test('@w5-m2-full definition is inert, calls really execute, formal proof survives refresh, readonly replay and parent summary', async ({ page }, info) => {
  ...
  await page.screenshot({ path: info.outputPath('w5m2-default.png'), fullPage: true });
  await run(page); expect((await session(page)).lastRun.state).toBe('call-missing'); expect((await session(page)).lastCanonicalTrace).toHaveLength(1);
  await page.getByRole('button', { name: '逐步查看已保存轨迹' }).click(); await expect(page.getByText(/第 1 行：定义.*尚未执行/)).toBeVisible();
  ...
  const completed = await saved(page); expect(completed.schemaRevision).toBe(14); expect(completed.missionCompletionEvidence['w5-m2'].kind).toBe('formal-v3'); expect(completed.works['w5-m2-sanqing-function-record'].run.completed).toBe(true);
  ...
  await page.getByRole('button', { name: '清空学习数据', exact: true }).click(); await page.getByLabel('输入“清空”以确认').fill('清空'); const backup = page.waitForEvent('download'); await page.getByRole('button', { name: '备份并清空', exact: true }).click(); await backup; await expect.poll(async () => (await saved(page)).missions['w5-m2']).toBeUndefined();
  await parent(page); await page.getByLabel('选择进度文件').setInputFiles({ name: 'w5m2.json', mimeType: 'application/json', buffer: bytes }); await expect.poll(async () => (await saved(page)).missionCompletionEvidence['w5-m2']?.kind).toBe('formal-v3'); await open(page);
  await page.goto('./#/mission/w5-m3'); await expect(page.getByRole('heading', { name: '祈雨赌胜', exact: true })).toBeVisible(); expect(errors).toEqual([]);
});

test('@w5-m2-keyboard visible CodeMirror input, blank lines, Tab exit and no overflow', async ({ page }) => {
  await seed(page); await open(page); const editor = page.getByLabel('W5-M2 Python 函数代码');
  ...
  await editor.press('Tab'); await expect(page.getByRole('button', { name: '在定义后调用一次' })).toBeFocused();
...
for (const stage of ['run', 'validation', 'observation', 'work', 'completion'] as const) test(`@w5-m2-storage ${stage} failure retries the original transaction without duplicate execution`, async ({ page }) => {
  await seed(page); await open(page);
  if (stage === 'observation') await run(page);
  if (stage === 'validation') await setCode(page, 'def record_sanqing(value):\n    record_arrival()');
  if (stage === 'work' || stage === 'completion') await setCode(page, SOLVED);
  await page.evaluate(({ mode, stage }) => localStorage.setItem(mode, `fail-w5-m2-${stage === 'validation' ? 'run' : stage}`), { mode: MODE, stage });
...
test('@w5-m2-external stale tab cannot overwrite and explicitly reloads the newer function draft', async ({ page, context }) => {
  ...
  const second = await context.newPage(); await open(second); await second.getByRole('button', { name: '交换函数内两步' }).click(); await expect.poll(async () => (await session(second)).pythonCode).toContain('record_names()\n    record_arrival()');
  ...
  await page.getByRole('button', { name: '载入其他标签页进度' }).click(); await expect.poll(async () => (await session(page)).pythonCode).toBe(external.sessions['w5-m2'].pythonCode);
});

test('@w5-m2-security actual built Worker rejects imports, recursion, properties and files while preserving input-derived line/scope trace', async ({ page }) => {
  await seed(page); await open(page); const before = await saved(page);
  const workers = readdirSync('dist-e2e/assets').filter((file) => /^weekFiveFunctionPython\.worker-.*\.js$/.test(file)); expect(workers).toHaveLength(1);
  const result = await page.evaluate(async ({ file, solved }) => {
    ...
    for (const code of ['import os', 'def record_sanqing():\n    record_sanqing()\nrecord_sanqing()', 'def record_sanqing():\n    record_arrival.__call__()', 'def record_sanqing():\n    open("x")', solved, `${solved}\nrecord_names()`]) output.push(await new Promise((resolve) => { worker.onmessage = (event) => resolve(event.data); worker.postMessage({ type: 'run', requestId: ++requestId, code }); }));
    worker.terminate(); return output;
  }, { file: workers[0], solved: SOLVED });
  expect(result.slice(0, 4).map((item) => item.type)).toEqual(Array(4).fill('error')); expect(result[4].trace).toHaveLength(4); expect(result[4].trace[2]).toMatchObject({ line: 2, scope: 'inside' }); expect(result[5].trace.at(-1)).toMatchObject({ line: 6, scope: 'outside' }); expect(await saved(page)).toEqual(before);
});

test('@w5-m2-assets missing temple background blocks proof and retry completes without rerunning Python', async ({ page }) => {
  await seed(page); let requests = 0; const pattern = /sanqing-courtyard-background\.webp/; await page.route(pattern, async (route) => { requests += 1; await route.fulfill({ status: 503, body: 'unavailable' }); });
  await open(page); await setCode(page, SOLVED); await run(page); expect((await session(page)).lastRun.completed).toBe(true); expect((await saved(page)).missions['w5-m2']).toBeUndefined(); expect(requests).toBeGreaterThan(0);
  await page.unroute(pattern); await page.getByRole('button', { name: '重试加载三清观场景' }).click(); await expect(page.getByRole('dialog', { name: '闯关成功' })).toBeVisible({ timeout: 30_000 }); expect((await session(page)).totalRuns).toBe(1);
});

test('@w5-m2-runtime load failure is infrastructure, preserves the draft, and a later real Worker run recovers', async ({ page }) => {
  ...
  await page.goto('./#/mission/w5-m2'); await expect(page.getByLabel('W5-M2 Python 函数代码')).toBeVisible(); await expect(runButton(page)).toBeEnabled({ timeout: 30_000 }); await expect(page.getByRole('status', { name: 'Python 运行环境暂不可用' })).toBeVisible({ timeout: 30_000 });
  ...
  await page.unroute(pattern); await run(page); expect((await session(page)).lastRun.state).toBe('call-missing');
});

test('@w5-m2-runtime timeout counts one attempted run and explicit cancel discards a delayed result', async ({ page }) => {
  await seed(page); let delivered = 0; const pattern = /weekFiveFunctionPython\.worker-[^/]+\.js/;
...
test('@w5-m2-corrupt restores the formal snapshot, preserves damaged source, and readonly replay remains exact', async ({ page }) => {
  ...
  await page.evaluate((mode) => localStorage.setItem(mode, 'corrupt-w5-m2-current'), MODE); await page.reload(); await expect(page.getByText('学习进度已经安全恢复')).toBeVisible();
  const recovered = await saved(page); expect(recovered.recovery.source).toBe('snapshot'); expect(recovered.works['w5-m2-sanqing-function-record']).toEqual(completed.works['w5-m2-sanqing-function-record']);
  await parent(page); const download = page.waitForEvent('download'); await page.getByRole('button', { name: '下载损坏原文' }).click(); const damaged = readFileSync((await (await download).path())!, 'utf8'); expect(JSON.parse(damaged).current).toBe('{broken w5-m2 current');
  await open(page); const before = await saved(page); await page.getByRole('button', { name: '真实回放函数作品' }).click(); await expect(page.getByRole('dialog', { name: '闯关成功' })).toBeVisible({ timeout: 30_000 }); expect(await saved(page)).toEqual(before);
});

test('@w5-m2-legacy revision13 completion stays legacy until a real run upgrades proof and keeps history', async ({ page }) => {
  const old = JSON.parse(formalW5M1Prerequisite()); old.schemaRevision = 13; old.missions['w5-m2'] = { status: 'completed', stars: 2, hintsUsed: 1, attempts: 2, completedAt: '2026-09-11T00:00:20.000Z' };
  await seed(page, JSON.stringify(old)); await page.goto('./#/mission/w5-m2'); await expect(page.getByLabel('W5-M2 Python 函数代码')).toBeVisible(); expect((await saved(page)).missionCompletionEvidence['w5-m2'].kind).toBe('legacy-replay-only');
  await setCode(page, SOLVED); await runButton(page).click(); await expect(page.getByRole('dialog', { name: '闯关成功' })).toBeVisible({ timeout: 30_000 }); const current = await saved(page); expect(current.missions['w5-m2']).toEqual(old.missions['w5-m2']); expect(current.missionCompletionEvidence['w5-m2'].kind).toBe('formal-v3');
});

test('@w5-m2-assets alternate lazy route retries without creating a session or hidden completion', async ({ page }) => {
  ...
  const primary = manifest['src/components/WeekFiveFunctionExperience.tsx'].file; const retry = manifest['src/components/WeekFiveFunctionExperience.tsx?retry=1'].file; expect(primary).not.toBe(retry);
  ...
  await page.goto('./#/mission/w5-m2'); await expect(page.getByRole('button', { name: '重新加载页面', exact: true })).toBeVisible(); expect((await saved(page)).sessions['w5-m2']).toBeUndefined(); expect(blocked).toBeGreaterThan(0);
  await page.unroute(`**/${primary}`); await page.getByRole('button', { name: '重新加载页面', exact: true }).click(); await expect(page.getByLabel('W5-M2 Python 函数代码')).toBeVisible(); await run(page); expect((await session(page)).lastRun.state).toBe('call-missing');
});

test('@w5-m2-cold cold closure, media, built Worker, local Pyodide, first result and warm result meet fixed budgets', async ({ page, context }, info) => {
  ...
  const closureFiles = [...collectRuntimeClosure(manifest, 'src/components/WeekFiveFunctionExperience.tsx')].map((key) => manifest[key]?.file).filter((file): file is string => typeof file === 'string' && file.endsWith('.js'));
  const closureBytes = closureFiles.reduce((sum, file) => sum + statSync(new URL(`../dist-e2e/${file}`, import.meta.url)).size, 0);
  const workers = readdirSync(new URL('../dist-e2e/assets/', import.meta.url)).filter((file) => /^weekFiveFunctionPython\.worker-.*\.js$/.test(file)); expect(workers).toHaveLength(1);
  ...
  await page.goto('./#/mission/w5-m2');
  await expect(page.locator('.week-five-function-experience > header').getByRole('heading', { name: '定义是准备，调用才执行' })).toBeVisible();
  await expect(page.getByRole('status', { name: 'Python 运行环境已准备' })).toBeVisible({ timeout: 25_000 });
  const readyAt = performance.now();
  const resources = await page.evaluate(() => performance.getEntriesByType('resource').map((entry) => entry.name));
  expect(resources.some((url) => url.includes('weekFiveFunctionPython.worker-'))).toBe(true);
  ...
  await run(page);
  await expect(page.getByRole('status', { name: '函数运行结果' })).toContainText('没有调用', { timeout: 25_000 });
  ...
  const mediaResponses = responses.filter((response) => new URL(response.url()).pathname.endsWith('/assets/week-five-temple/sanqing-courtyard-background.webp'));
  ...
  expect(closureBytes + workerBytes).toBeLessThanOrEqual(WEEK_FIVE_FUNCTION_MAX_LAZY_BYTES);
  ...
  const metricsPath = info.outputPath(`w5m2-cold-metrics-${info.project.name}.json`);
  ...
  await info.attach(`w5m2-cold-metrics-${info.project.name}.json`, { path: metricsPath, contentType: 'application/json' });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await page.screenshot({ path: info.outputPath('w5m2-cold.png'), fullPage: true });
