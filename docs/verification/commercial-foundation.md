...
- Result: 79/79 Playwright scenarios passed with no skipped project in 4.3 minutes.
- Non-browser gates: 444/444 Vitest tests, 18/18 bundle-script tests, 26/26 asset-manifest tests, TypeScript `--noEmit`, and `git diff --check` passed.
...
Lazy failures are evidenced by component, not generalized from one chunk. The existing five-project browser path covers the mission Blockly tool chunk. Separate desktop Chromium paths return 503 for the parent-entry chunk and the recovery-notice chunk, prove the rest of the application stays visible, and use the displayed reload action to recover. The parent data-tools chunk has the same local boundary and unit-level boundary coverage, but does not have its own 503 browser scenario. External-tab reload now replaces status, persistence, load error, corrupt-source download, and corrupt-envelope error together; a browser path corrupts current progress from another page, supplies a valid snapshot and newer revision, then proves explicit reload shows recovery and preserves the damaged source for parent download.
...
- Homepage entry static JS: 106.4 KiB gzip / 180 KiB budget.
- Conservative homepage total: 416.3 KiB / 650 KiB budget.
- Approved lazy Phaser chunk: 1,168.4 KiB raw / 1,600 KiB ceiling.
- GameScene closure: 1,507.1 KiB raw / 431.6 KiB gzip.
- Blockly workspace closure: 1,043.1 KiB raw / 303.0 KiB gzip.
- Python editor closure: 697.5 KiB raw / 230.6 KiB gzip.
- AI lab closure: 338.2 KiB raw / 109.7 KiB gzip.
- Real cold Dragon Palace response bodies: 2,577,186 B, below the fixed 2.5 MiB budget.

The browser matrix covers privacy acknowledgement, real Blockly failure/correction/battle/persistence, parent PIN/export, V1/V2 migration, malformed import rollback, snapshot and corrupt-source recovery, backup-before-clear, keyboard/focus paths, reduced motion, mute controls, the exact lazy-chunk paths described above, 320 px overflow, touch targets, GitHub Pages base paths, console/page/request health, cross-tab conflict/rebase, simultaneous-writer serialization, no-lock fail-closed behavior, CAS conflict without a storage event, external corrupt reload metadata, and prevention of stale revival after clear.
