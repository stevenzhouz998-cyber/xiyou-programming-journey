...
| assets/week-six-records/flaming-mountain-background.webp | 85c40c368dc7ab61ec5abe56fcf082ddccbc81923ecfe907ae7a5d99825deadc | Child-safe Flaming Mountains learning pavilion, static story setting independent of executed records | OpenAI built-in image_gen | [Prompt W6M1-001](#prompt-w6m1-001-flaming-mountain-background) | 1536x1024 | generated in-project with built-in image_gen; provenance verified | w6-m1 WeekSixRecordsScene; w6-m2 WeekSixClassificationScene; w6-m3 WeekSixPromptScene | visual-qa-passed |
...
The existing W6M1-001 raster is reused unchanged in `WeekSixClassificationScene` and `WeekSixPromptScene` as the story setting for the child's read-only source material. Purpose, generation prompt, dimensions, hash and provenance remain the registered original. No generated labels or solved outcome are baked into the scene. The primary agent inspected the original image and the integrated desktop / 320px scene and classification layout after correcting clipped evidence text; evidence is under `artifacts/w6m2-independent/` (`desktop-layout.png`, `narrow-layout.png`, and `*-classification-viewport.png`). The M1, M2 and M3 source slots are enforced by the asset inventory contract, including rejection of a missing slot or an unapproved image reference. These development images establish asset-slot QA only; final production browser evidence is recorded in each mission acceptance report.
