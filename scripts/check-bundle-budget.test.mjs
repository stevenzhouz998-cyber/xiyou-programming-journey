...
test('counts the unique real built W4-M3 Worker in its closure and fails closed on missing, duplicate, or oversized output', () => {
  ...
  const allWorkerFiles = emittedFiles.filter(file => /^assets\/(?:weekFour(?:Branch|List|Boss)|weekFive(?:Monks|Function))Python\.worker-[A-Za-z0-9_-]+\.js$/.test(file));
...
test('bounds both W4-M4 entry closures including their unique real Worker and rejects oversized or missing Worker bytes', () => {
  ...
  const all = [...new Set([...Object.values(manifest).map(c=>c.file).filter(f=>f?.endsWith('.js')), ...emitted.filter(f=>/(?:weekFour(?:Branch|List|Boss)|weekFive(?:Monks|Function))Python\.worker-/.test(f))])];
