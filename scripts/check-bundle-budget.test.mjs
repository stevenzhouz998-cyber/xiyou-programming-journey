...
test('counts the unique real built W4-M3 Worker in its closure and fails closed on missing, duplicate, or oversized output', () => {
  ...
  const allWorkerFiles = emittedFiles.filter(file => /^assets\/(?:weekFour(?:Branch|List|Boss)|weekFive(?:Monks|Function|Weather|Decomposition|StoryOrchestration)|weekSixRecords)Python\.worker-[A-Za-z0-9_-]+\.js$/.test(file));
...
test('keeps mission, parent, Ruyi, and Four Seas CSS out of the homepage entry closure', () => {
  ...
  const parentPageSource = readFileSync(new URL('../src/components/ParentPage.tsx', import.meta.url), 'utf8');
  ...
  assert.match(appSource, /lazy\(\(\)\s*=>\s*import\(['"]\.\/components\/ParentPage['"]\)/);
  assert.match(parentPageSource, /lazy\(\(\)\s*=>\s*import\(['"]\.\/ParentAccessGate['"]\)/);
  assert.match(parentPageSource, /lazy\(\(\)\s*=>\s*import\(['"]\.\/ParentDataTools['"]\)/);
...
test('bounds both W4-M4 entry closures including their unique real Worker and rejects oversized or missing Worker bytes', () => {
  ...
  const all = [...new Set([...Object.values(manifest).map(c=>c.file).filter(f=>f?.endsWith('.js')), ...emitted.filter(f=>/(?:weekFour(?:Branch|List|Boss)|weekFive(?:Monks|Function|Weather|Decomposition|StoryOrchestration)|weekSixRecords)Python\.worker-/.test(f))])];
...
test('bounds both W4-M5 entry closures including their unique real Worker and rejects oversized or missing Worker bytes', () => {
  ...
  const result=analyzeManifest(manifest,gzip,raw,emitted);
  for(const root of ['src/components/WeekFourBossExperience.tsx','src/components/WeekFourBossExperience.tsx?retry=1']) {
    assert.equal(result.closures[root].workerFile,workers[0]);
    assert.ok(result.closures[root].rawBytes<=3*1024*1024);
  }
  assert.throws(()=>analyzeManifest(manifest,gzip,{...raw,[workers[0]]:3*1024*1024},emitted),/WeekFourBossExperience.*closure exceeds/);
  assert.throws(()=>analyzeManifest(manifest,gzip,raw,emitted.filter(f=>f!==workers[0])),/Worker.*exactly one|missing/);
});
...
test('bounds both W5-M1 entry closures including their unique real Worker and rejects oversized or missing Worker bytes', () => {
  ...
  const result=analyzeManifest(manifest,gzip,raw,emitted);
  for(const root of ['src/components/WeekFiveMonksExperience.tsx','src/components/WeekFiveMonksExperience.tsx?retry=1']) {
    assert.equal(result.closures[root].workerFile,workers[0]);
    assert.ok(result.closures[root].rawBytes<=3*1024*1024);
  }
  assert.throws(()=>analyzeManifest(manifest,gzip,{...raw,[workers[0]]:3*1024*1024},emitted),/WeekFiveMonksExperience.*closure exceeds/);
  assert.throws(()=>analyzeManifest(manifest,gzip,raw,emitted.filter(f=>f!==workers[0])),/Worker.*exactly one|missing/);
});
...
test('bounds both W5-M2 entry closures including their unique real Worker and rejects oversized or missing Worker bytes', () => {
  ...
  const result=analyzeManifest(manifest,gzip,raw,emitted);
  for(const root of ['src/components/WeekFiveFunctionExperience.tsx','src/components/WeekFiveFunctionExperience.tsx?retry=1']) {
    assert.equal(result.closures[root].workerFile,workers[0]);
    assert.ok(result.closures[root].rawBytes<=3*1024*1024);
  }
  assert.throws(()=>analyzeManifest(manifest,gzip,{...raw,[workers[0]]:3*1024*1024},emitted),/WeekFiveFunctionExperience.*closure exceeds/);
  assert.throws(()=>analyzeManifest(manifest,gzip,raw,emitted.filter(f=>f!==workers[0])),/Worker.*exactly one|missing/);
});
...
test('bounds both W5-M3 entry closures including their unique real Worker and rejects oversized or missing Worker bytes', () => {
  ...
  const result=analyzeManifest(manifest,gzip,raw,emitted);
  for(const root of ['src/components/WeekFiveWeatherExperience.tsx','src/components/WeekFiveWeatherExperience.tsx?retry=1']) {
    assert.equal(result.closures[root].workerFile,workers[0]);
    assert.ok(result.closures[root].rawBytes<=3*1024*1024);
  }
...
test('bounds both W5-M5 entry closures including their unique real Worker and rejects oversized or missing Worker bytes', () => {
  const manifest = JSON.parse(readFileSync('dist/.vite/manifest.json', 'utf8'));
  const emitted = readdirSync('dist/assets').map(name => `assets/${name}`);
  const all = [...new Set([...Object.values(manifest).map(c=>c.file).filter(f=>f?.endsWith('.js')), ...emitted.filter(f=>/(?:weekFour(?:Branch|List|Boss)|weekFive(?:Monks|Function|Weather|Decomposition|StoryOrchestration)|weekSixRecords)Python\.worker-/.test(f))])];
  const raw = Object.fromEntries(all.map(f=>[f,statSync(`dist/${f}`).size]));
  const gzip = Object.fromEntries(all.map(f=>[f,gzipSync(readFileSync(`dist/${f}`)).byteLength]));
  const workers=emitted.filter(f=>/weekFiveStoryOrchestrationPython\.worker-/.test(f));assert.equal(workers.length,1);
  const result=analyzeManifest(manifest,gzip,raw,emitted);
  for(const root of ['src/components/WeekFiveStoryOrchestrationExperience.tsx','src/components/WeekFiveStoryOrchestrationExperience.tsx?retry=1']) {
    assert.equal(result.closures[root].workerFile,workers[0]);
    assert.ok(result.closures[root].rawBytes<=3*1024*1024);
  }
  assert.throws(()=>analyzeManifest(manifest,gzip,{...raw,[workers[0]]:3*1024*1024},emitted),/WeekFiveStoryOrchestrationExperience.*closure exceeds/);
  assert.throws(()=>analyzeManifest(manifest,gzip,raw,emitted.filter(f=>f!==workers[0])),/Worker.*exactly one|missing/);
});

test('keeps parent reports and deferred writes outside startup and asset-only scenes independent of progress context', () => {
  const manifest = JSON.parse(readFileSync('dist/.vite/manifest.json', 'utf8'));
  const entry = bundleBudget.collectStaticClosure(manifest, 'index.html');
  const parent = 'src/components/ParentPage.tsx';
  assert.ok(manifest[parent]?.isDynamicEntry, 'parent page remains a real lazy entry');
  assert.ok(!entry.has(parent));
  for (const prefix of ['_parent-report-', '_parent-data-', '_storage-write-']) {
    const key = Object.keys(manifest).find(key => key.startsWith(prefix));
    assert.ok(key, `${prefix} must be present in the build`);
    assert.ok(!entry.has(key), `${prefix} must not be downloaded with the homepage`);
  }
