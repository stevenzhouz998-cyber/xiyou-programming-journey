...
export const COLD_LOAD_ROUTE_CLOSURE_BUDGETS = Object.freeze({
  ...
  'src/components/WeekFiveStoryOrchestrationExperience.tsx': WEEK_FIVE_STORY_ORCHESTRATION_MAX_LAZY_BYTES,
...
const COLD_LOAD_ROUTE_STATIC_ISOLATION = Object.freeze({
  ...
  'src/components/WeekFiveStoryOrchestrationExperience.tsx': [
    'src/components/WeekFiveStoryOrchestrationPythonEditor.tsx',
    'src/components/WeekFiveStoryOrchestrationScene.tsx',
    'src/workers/weekFiveStoryOrchestrationPython.worker.ts',
  ],
});
...
export function analyzeManifest(manifest, gzipSizes, rawSizes = {}, emittedFiles) {
  ...
  for (const [root, maxBytes] of Object.entries(COLD_LOAD_ROUTE_CLOSURE_BUDGETS)) {
    ...
    if (isBranchRoute || isListRoute || isBossRoute || isMonksRoute || isFunctionRoute || isWeatherRoute || isDecompositionRoute || isStoryRoute) {
...
async function main() {
  ...
  const branchWorkerFiles = distFiles.filter((file) => /^assets\/(?:weekFour(?:Branch|List|Boss)|weekFive(?:Monks|Function|Weather|Decomposition|StoryOrchestration))Python\.worker-[A-Za-z0-9_-]+\.js$/.test(file));
