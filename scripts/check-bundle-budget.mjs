...
export const WEEK_FIVE_MONKS_MAX_LAZY_BYTES = 3 * 1024 * 1024;
...
export const WEEK_FIVE_DECOMPOSITION_MAX_LAZY_BYTES = 3 * 1024 * 1024;
const WEEK_FIVE_DECOMPOSITION_ROUTE_ROOTS = ['src/components/WeekFiveDecompositionExperience.tsx', 'src/components/WeekFiveDecompositionExperience.tsx?retry=1'];
...
const WEEK_FOUR_LIST_ROUTE_ROOTS = ['src/components/WeekFourListExperience.tsx', 'src/components/WeekFourListExperience.tsx?retry=1'];
...
export const COLD_LOAD_ROUTE_CLOSURE_BUDGETS = Object.freeze({
  ...
  'src/components/WeekFourListExperience.tsx': WEEK_FOUR_LIST_MAX_LAZY_BYTES,
  'src/components/WeekFourBossExperience.tsx': WEEK_FOUR_BOSS_MAX_LAZY_BYTES,
  'src/components/WeekFiveMonksExperience.tsx': WEEK_FIVE_MONKS_MAX_LAZY_BYTES,
  ...
  'src/components/WeekFiveDecompositionExperience.tsx': WEEK_FIVE_DECOMPOSITION_MAX_LAZY_BYTES,
  'src/components/WeekFiveDecompositionExperience.tsx?retry=1': WEEK_FIVE_DECOMPOSITION_MAX_LAZY_BYTES,
...
const COLD_LOAD_ROUTE_STATIC_ISOLATION = Object.freeze({
  ...
  'src/components/WeekFiveFunctionExperience.tsx': [
  ...
  'src/components/WeekFiveDecompositionExperience.tsx': [
    'src/components/WeekFiveDecompositionPythonEditor.tsx',
    'src/components/WeekFiveDecompositionScene.tsx',
    'src/workers/weekFiveDecompositionPython.worker.ts',
  ],
});
...
export function analyzeManifest(manifest, gzipSizes, rawSizes = {}, emittedFiles) {
  ...
  for (const [root, maxBytes] of Object.entries(COLD_LOAD_ROUTE_CLOSURE_BUDGETS)) {
    ...
    for (const isolatedRoot of COLD_LOAD_ROUTE_STATIC_ISOLATION[isBranchRoute ? WEEK_FOUR_BRANCH_ROUTE_ROOTS[0] : isFunctionRoute ? WEEK_FIVE_FUNCTION_ROUTE_ROOTS[0] : isWeatherRoute ? WEEK_FIVE_WEATHER_ROUTE_ROOTS[0] : isDecompositionRoute ? WEEK_FIVE_DECOMPOSITION_ROUTE_ROOTS[0] : root] ?? []) {
    ...
    const files = [...new Set([...keys].map((key) => manifest[key].file).filter((file) => file?.endsWith('.js')))];
    let workerFile;
    if ((isBranchRoute || isListRoute || isBossRoute || isMonksRoute || isFunctionRoute || isWeatherRoute || isDecompositionRoute) && emittedFiles !== undefined) {
      if (!Array.isArray(emittedFiles) || emittedFiles.some((file) => typeof file !== 'string')) throw new Error('Bundle budget: W4-M3 Worker emitted file inventory is invalid.');
      const workers = emittedFiles.filter((file) => (isDecompositionRoute ? /^assets\/weekFiveDecompositionPython\.worker-[A-Za-z0-9_-]+\.js$/ : isWeatherRoute ? /^assets\/weekFiveWeatherPython\.worker-[A-Za-z0-9_-]+\.js$/ : isFunctionRoute ? /^assets\/weekFiveFunctionPython\.worker-[A-Za-z0-9_-]+\.js$/ : isMonksRoute ? /^assets\/weekFiveMonksPython\.worker-[A-Za-z0-9_-]+\.js$/ : isBossRoute ? /^assets\/weekFourBossPython\.worker-[A-Za-z0-9_-]+\.js$/ : isListRoute ? /^assets\/weekFourListPython\.worker-[A-Za-z0-9_-]+\.js$/ : /^assets\/weekFourBranchPython\.worker-[A-Za-z0-9_-]+\.js$/).test(file));
      if (workers.length !== 1) throw new Error(`Bundle budget: W4-M3 Worker requires exactly one emitted file; found ${workers.length === 0 ? 'missing' : `duplicate ${workers.length}`}.`);
      [workerFile] = workers;
    ...
    if (isBranchRoute || isListRoute || isBossRoute || isMonksRoute || isFunctionRoute || isWeatherRoute || isDecompositionRoute) {
...
async function main() {
  ...
  const branchWorkerFiles = distFiles.filter((file) => /^assets\/(?:weekFour(?:Branch|List|Boss)|weekFive(?:Monks|Function|Weather|Decomposition))Python\.worker-[A-Za-z0-9_-]+\.js$/.test(file));
