...
export function verifyRequiredWeekSixRecordsInventory({ manifestRows, publicFiles, promptRecords = [], source, classificationSource, promptSource, factCheckSource, mode = 'check' }) {
  ...
  if (typeof factCheckSource !== 'string' || !factCheckSource.includes(`assetUrl('/${path}')`) || (factCheckSource.match(/<img\b/g) ?? []).length !== 1) throw new Error('Asset manifest: W6-M4 must render the approved background through assetUrl.');
  return verifyAssetManifest({ manifestRows: rows, publicFiles: files, promptRecords: promptRecordsForRows(promptRecords, rows), mode });
}
